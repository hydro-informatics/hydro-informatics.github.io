---
description: Guide pour l'exécution des simulations morphodynamiques TELEMAC-GAIA et l'analyse des résultats du transport des sédiments, y compris la charge de fond, la charge en suspension et les sorties d'évolution du lit.
---
...
**rGaia-steady2d.slf** contient les variables morphodynamiques définies dans le fichier de direction de Gaia (p. ex. élévation du fond, taux de transport de la charge de fond, concentrations de sédiments). Les variables correspondent au mot clé **VARIABLES POUR PRINTOUTS GRAPHIQUES** à `gaia-morphodynamics.cas`.
...
Aussi la profondeur de l'eau, la vitesse du débit (vecteurs) et {term}`Changement du terrain <Topographic change>` devraient être analysées (en QGIS ou BlueKenue) puisque Gaia modifie les élévations du lit de rivière. Par exemple, si le modèle prédit {term}`Changement du terrain <Topographic change>` sous la forme d'une érosion profonde de 10 m au débit de base, les définitions de mots clés pour le {ref}`riverbed <gaia-active-lyr>` devraient être révisées. De même, les paramètres hydro-morphodynamiquement pertinents tels que {ref}`friction <c-friction>`, ou {ref}`direction and magnitude (bedload) <gaia-dir>` correcteurs doivent être vérifiés.
...
- [ ] La direction de la charge de fond s'harmonise avec la direction du flux principal (avec les écarts attendus dans les courbes)
...
** Charge en suspension (si activée):**
...
(bl-calibration)=
### Paramètres d'étalonnage de la charge de fond

La liste suivante de paramètres peut être envisagée pour l'étalonnage de la charge de fond dans Gaia:
...
* **Representative roughness length** $k'_{s}$ (cf. Equation {eq}`eq-cf-skin`) with the keyword **RATIO BETWEEN SKIN FRICTION AND MEAN DIAMETER** $\alpha_{ks}$ (default: $\alpha_{ks}$=`3.`). Note that this keyword is a multiplier of the mean grain diameter $D_{50}$; thus: $k'_{s}= \alpha_{ks} \cdot D_{50}$ (goes into Equation {eq}`eq-cf-skin`):
  ...
  * Augmenter $\alpha_{ks}$ augmente la friction cutanée et donc les taux de transport de la charge de fond.
...
* Le **MPM COEFFICIENT** peut être ajusté (par défaut: `8`):
  ...
  * Réduire pour diminuer les taux globaux de transport de la charge de fond.
...
### Paramètres d'étalonnage de charge en suspension
...
**Et ensuite?**
: Le modèle étalonné devra également être validé. La validation nécessite un autre ensemble de mesures de l'altitude du lit de rivière avant et après une autre inondation (c.-à-d. une autre carte spécifique à l'événement {term}`Changement du terrain <Topographic change>`). Hélas, les cartes {term}`Changement du terrain <Topographic change>` sont chères et il est rare d'avoir au moins trois {term}`Modèle numérique de terrain <DEM>`s de différents points dans le temps pour une section de rivière, ce qui permettrait la création de deux cartes {term}`Changement du terrain <Topographic change>`. Pour cette raison, l'ensemble de données d'étalonnage est souvent divisé en pratique. Par exemple, les 2/3 d'une carte {term}`Changement du terrain <Topographic change>` peuvent être utilisés pour l'étalonnage du modèle et les 1/3 pour la validation du modèle. Toutefois, ce partage fait que les deux ensembles de données ne sont pas statistiquement indépendants et que les chiffres de qualité de validation seront biaisés.
