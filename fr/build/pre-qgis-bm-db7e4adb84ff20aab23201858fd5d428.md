...
This tutorial uses an application-ready {term}`Modèle numérique de terrain <DEM>` in {term}`GeoTIFF` {ref}`raster` format that stems from a {term}`Lidar` point cloud. The {term}`Modèle numérique de terrain <DEM>` raster provides height (Z) information from a section of a gravel-cobble bed river in South-East Germany, which constitutes the baseline for the computational grids featured in the next sections. To get the provided DEM in the *QGIS* project:
...
* Pour faciliter la délimitation de certaines régions de l'écosystème fluvial plus tard, ajoutez un {ref}`satellite imagery basemap <basemap>` (carrelage XYZ) sous le {term}`Modèle numérique de terrain <DEM>` et personnalisez la symbolique du calque.
...
* Facultatif : **Breaklines des rampes de blocs** :
  ...
  * Pour le champ **LineType**, utilisez des valeurs de texte telles que **hardline sss** (ou n'importe quoi d'autre - l'exemple se réfère au mot allemand <u>S</u>chütt<u>s</u>tein<u>s</u>chwelle).
...
Le style de calque par défaut est **Single Symbol**. Pour une meilleure représentation, double-cliquez sur le calque des lignes de rupture, allez à l'onglet **Symbole** et sélectionnez **Categorized** (ou **Graduated**) au lieu de **Symbole unique** (en haut de la fenêtre **Propriétés de feuille**). Dans le champ **Value**, sélectionnez **LineType**, puis cliquez sur le bouton **classify** au bas de la fenêtre **Layer Properties**. La listbox va maintenant afficher les valeurs *LineType*.

```{admonition} Draw boundaries of complex DEMs...
:class: tip
Dessiner des limites manuellement autour de grand {term}`Modèle numérique de terrain <DEM>`s peut être très long, en particulier, si les données brutes sont un nuage de point et pas encore converti en {ref}`raster`.
...
Les limites des liquides définissent les endroits où les conditions hydrauliques, telles qu'une relation de décharge ou d'écoulement par étape, s'appliquent aux limites d'écoulement (en amont) et d'écoulement (en aval) du modèle. Ainsi, un modèle de rivière fonctionnel nécessite au moins une limite d'écoulement (ligne) où le débit massique dans le modèle et une limite d'écoulement (ligne) où les flux massiques quittent le modèle. À cette fin, {ref}`create-line-shp` appelé **liquid-boundarys.shp** et définir **deux champs de données texte** nommé **type** et **stringdef**. Assurez-vous que **snapping** est toujours **enabled** (comme expliqué ci-dessus dans la section {ref}`boundary`) et **Toggle (Start) Editing** <img src="../img/qgis/yellow-pen.png"> the new **liquide-boundaries.shp**. Puis dessinez deux lignes :
...
* Le {term}`Système de coordonnées <CRS>` <img src="../img/qgis/sym-crs.png"> correspond à l'Allemagne Zone 4 ({ref}`see project CRS <start-qgis>`)
