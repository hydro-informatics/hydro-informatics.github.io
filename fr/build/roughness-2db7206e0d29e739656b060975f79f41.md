...
* Définissez le Système de coordonnées que votre {ref}`basemap <basemap>` / {term}`DEM` data use; pour cet exemple, entrez `UTM zone 33N` et sélectionnez *UTM zone 33N (WGS84)* (EPSG 32633), ce qui n'est pas un grand choix en raison de sa faible précision, mais il fera le travail pour ce tutoriel.
