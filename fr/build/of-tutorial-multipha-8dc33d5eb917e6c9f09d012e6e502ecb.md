...
* *ChâtelatedMeshControls* est une sous-diction de contrôles pour les mailles castellées.
* *snapControls* est un sous-dictionnaire des commandes pour le snapping de surface.
* *addLayersControls* est un sous-dictionnaire de contrôles pour l'addition de couches.
...
1. {ref}`Layering <of-mesh-layer>`: crée des couches supplémentaires dans la région frontalière.
1. {ref}`Mesh quality <of-mesh-quality>`: contrôler et vérifier la qualité du maillage.
...
* *maxLocalCells* : ce paramètre est utilisé dans le cas d'un fonctionnement parallèle et définit le nombre maximal de cellules pour chaque processeur.
* * *CellsBetweenLevels* : évite les changements soudains de la taille des cellules, ce qui signifie que les changements consécutifs de niveau de raffinement se rapprochent.
...
* *Tolérance* : indique la région à l'intérieur de laquelle les points sont attirés par la surface.
...
* *ExplicitéCaractéristiquesSnap* : si elle est activée, elle active le maillage sur les fonctionnalités définies dans les fichiers *eMesh*.
...
* *Ratio d'expansion*: nécessaire pour calculer la taille relative à l'épaisseur prescrite de la première ou de la dernière couche.
...
* *minAxe médialAngle*: indique l'angle minimal pour sélectionner les points de l'axe médial
...
* <span style="color: #f2003c ">***maxNonOrtho***</span>: angle maximum de non-orthogonalité du visage, calculé comme le produit à point normalisé du vecteur de surface d'une cellule $i$ et le vecteur controïde à centroïde de deux cellules voisines $i$ et $j$.
* <span style="color: #e68a19 ">***maxBoundarySkewness***</span>: limite maximale.
* <span style="color: #e68a19 ">***maxInternalSkewness***</span>: maximum de skewness de la face interne.
* <span style="color: #e68a19 ">***maxConcave***</span>: concavité cellulaire maximale pour vérifier le visage des angles intérieurs.
* <span style="color: #f2003c ">***minVol***</span>: volume minimal de pyramide cellulaire, calculé comme le produit de point du vecteur de surface cellulaire et du centre cellulaire vers le vecteur de pic pyramidal.
* <span style="color: #e68a19 ">***minArea***</span>: zone de taille minimale.
...
* Lancez la commande `surfaceFeatures` pour créer les fichiers *.obj* et *.eMesh* à partir des fichiers initialement importés {term}`STL`. Ces fichiers sont stockés dans le dossier *extensionFeatureEdgeMesh* (canal/constante/extensionFeatureEdgeMesh).
```
