---
description: Introduction au couplage de l'hydrodynamique TELEMAC avec la morphodynamique GAIA pour simuler la charge de fond et le transport des sédiments en suspension dans les rivières, les lacs et les estuaires.
---
...
## Terminologie
Une simulation hydromorphodynamique implique la modélisation de processus axés sur le ruissellement **{term}`Transport solide <Sediment transport>`**. Les sections précédentes de ce livre électronique portent sur l'hydrodynamique définie comme *l'étude des liquides en mouvement* et la présente section porte sur **morphodynamique** définie comme **l'étude des changements liés au temps dans les formes des lits alluviaux et leurs processus sous-jacents**.

(gaia-seditrans)=
## Modes de transport des sédiments

TELEMAC has a dedicated module called Gaia for modeling morphodynamics. Gaia enables modeling sediment transport and morphological evolution (i.e., {term}`Changement du terrain <Topographic change>`) in rivers, lakes, and estuaries. It comes with particular routines to consider a spatio-temporal variation of grain sizes, grading curves, and riverbed layering for simulating sediment transport in the form of **{term}`charge de fond <Bedload>` (coarse sediment)** and/or **{term}`charge en suspension <Suspended load>` (fine sediment)**. {term}`charge de fond <Bedload>` is calculated by solving semi-empiric equations, such as the {cite:t}`meyer-peter_formulas_1948` formula (read more later in this tutorial). {term}`charge en suspension <Suspended load>` is modeled by solving the {term}`Advection`-{term}`Diffusion` equations (typically, the {term}`Moyenne de Reynolds <RANS>` form), which require closures for sediment erosion and deposition fluxes. {numref}`Figure %s <bl-vs-sl>` qualitatively illustrates the two basic modes of sediment transport in the form of suspended load and bedload. Whether a particle is transported in suspension or as bedload can also be determined by calculating of the {term}`Rouse number`.
...
Représentation qualitative de deux modes de transport des sédiments. À gauche : charge en suspension sous forme de particules fines se déplaçant avec le flux en vrac ; à droite : charge de fond sous forme de particules roulant, sautant ou glissant sur le lit de la rivière.
```

On distingue davantage les sédiments très fins, **cohésifs** et les sédiments plus grossiers, **non cohésifs**. En outre, Gaia explique l'évolution du lit à travers une solution itérative du {term}`Équation de Exner <Exner equation>` {cite:p}`exner_uber_1925` pour la conservation de masse.

Le recrutement de sédiments pour le transport de charge en suspension et de charge de fond nécessite un examen détaillé du lit de la rivière, qui sera fourni plus tard dans la section sur la définition de {ref}`the riverbed composition and the active layer <gaia-active-lyr>`.


(tm-coupling)=
## Couplage TELEMAC et Gaia

Le module morphodynamique Gaia peut être interne **couplé** avec les modèles hydrodynamiques Telemac2d (solution du {term}`Équations <Shallow water equations>`) ou Telemac3d (solution du Reynolds-moyenne {term}`Navier-Stokes (RANS) equations <Navier-Stokes equations>`). Cette section explique les types de couplage Telemac2d/Telemac3d (hydrodynamique) avec Gaia (morphodynamique).
...
: Un modèle entièrement couplé résout l'hydrodynamique {term}`Équations de Navier-Stokes <Navier-Stokes equations>` en même temps que les équations de transport des sédiments (c.-à-d. l'érosion et les flux de dépôt depuis et vers le lit de la rivière à travers le {term}`Équation de Exner <Exner equation>`). L'élévation du lit (c.-à-d. {term}`Changement du terrain <Topographic change>`) est calculée pour chaque pas de temps, ce qui conduit à ** longtemps de calcul**. En plus du couplage de l'hydrodynamique gravitationnelle (c.-à-d. flux en vrac le long des pentes de la vallée), {term}`Transport solide <Sediment transport>`, et {term}`Changement du terrain <Topographic change>`, un modèle peut également être couplé avec l'hydrodynamique des vagues (surface).
...
Modèle découplé
: Un modèle découplé alterne entre l'hydrodynamique et la morphodynamique (c'est-à-dire le {term}`Équation de Exner <Exner equation>`). Le lit de la rivière est considéré comme fixe lorsque les variables hydrodynamiques sont calculées, puis les changements d'altitude du lit sont calculés séparément en fonction du champ de débit calculé. Cette approche *synchrone* est plus efficace que le couplage complet.
