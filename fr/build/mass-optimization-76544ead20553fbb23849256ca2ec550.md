...
1. Pour un modèle **dry-initialisé** (par exemple, dans le fichier {ref}`steady 2d tutorial <telemac2d-steady>`), utiliser `5 5 5` en amont (préciser Q et H) et une limite `5 4 4` en aval (préciser H) dans le fichier `.cli` pour prescrire **décharges par les mots-clés `PRESCRIBED FLOWRATES` et `PRESCRIBED ELEVATIONS`, respectivement dans le fichier de direction (`.cas`).
   ...
   * La limite `5 5 5` du modèle initialisé à sec nécessite un {term}`Courbe d'étalonnage <Stage-discharge relation>` bien défini qui peut être établi en fonction du {ref}`1d hydraulics Python exercise <ex-1d-hydraulics>` et de la valeur optimale qui en résulte pour le mot-clé ** COEFFICIENT DES SOUNDARIES** dans le fichier de pilotage (voir ci-dessous).
...
1. Utilisez les mots-clés suivants pour prescrire des coefficients de rugosité aux limites qui correspondent à **mesuré {term}`Courbe d'étalonnage <Stage-discharge relation>`** et à l'hydraulique moyenne de section rétrocalculée:
