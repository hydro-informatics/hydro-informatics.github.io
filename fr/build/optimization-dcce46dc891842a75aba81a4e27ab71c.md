...
Des simulations instables peuvent se produire lorsque la condition {term}`Nombre de Courant <CFL>` est insuffisamment remplie. Pour s'assurer que la condition {term}`Nombre de Courant <CFL>` est respectée, activez le calcul à temps variable et utilisez le mot-clé **DESIRED COURANT** (valeur par défaut `1`), par exemple:
