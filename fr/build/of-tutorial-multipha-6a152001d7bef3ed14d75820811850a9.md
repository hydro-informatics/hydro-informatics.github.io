...
La surfaceCaractéristiquesDict extrait et écrit toutes les fonctionnalités de surface dans un fichier. Dans ce fichier, tous les fichiers {term}`STL` qui ont été enregistrés dans le dossier triSurface doivent être ajoutés comme suit:
...
Dans la section *affinementSurfaces* de l'exemple ci-dessus, différents niveaux de raffinement ont été fixés pour chaque élément constitutif. On trouvera ci-dessous un exemple détaillé du raffinement qui en résulte pour les éléments de fond de gravier et d'obstacle.
...
* <span style="color: #f2003c ">***maxNonOrtho***</span>: angle maximum de non-orthogonalité de la face, calculé comme le produit à point normalisé du vecteur de surface d'une cellule $i$ et le vecteur controïde à centroïde de deux cellules voisines $i$ et $j$.
