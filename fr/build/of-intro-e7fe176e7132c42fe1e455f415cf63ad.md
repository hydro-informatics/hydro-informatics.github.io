...
* Il effectue des calculs parallèles permettant de profiter pleinement des processeurs multicore modernes et des ordinateurs multiprocesseurs grâce à la norme [Interface de Passage de Message (MPI))](https://www.mpi-forum.org/).


```{admonition} Requirements for this tutorial
:class: attention

Pour réaliser ce tutoriel, assurez-vous qu'OpenFOAM est installé sur votre ordinateur (voir {ref}`OpenFOAM installation section <openfoam-install>`). Techniquement, une compréhension de base de la {term}`Équations de Navier-Stokes <Navier-Stokes equations>`, en particulier {term}`Moyenne de Reynolds <RANS>` et les fermetures de turbulences connexes, facilite la compréhension des descriptions et des hypothèses faites dans ce tutoriel. Et vous êtes prêt à partir.
```
...
* Informations générales sur l'exécution d'OpenFOAM, compilation, solveurs, modèles, génération de mailles et post-traitement dans le [OpenFOAM User Guide](https://doc.cfd.direct/openfoam/user-guide/) et [OpenFOAM Programr's Guide](https://doc.cfd.direct/openfoam/dev/)
...
* Des tutoriels vidéo comportant des instructions détaillées étape par étape peuvent être consultés à l'adresse suivante : [youtube.com/OpenFOAMJozsefnagy](https://www.youtube.com/@OpenFOAMJozsefNagy/)
* Notes de cours d'un cours de doctorat CFD avec logiciel openSource, disponible à : [tfd.chalmers.se](http://www.tfd.chalmers.se/~hani/kurser/OS_CFD/#YEAR_2022)
