...
* Pour obtenir le nombre de colonnes (valables) dans chaque fichier utiliser
  ...
  ```
* Maintenant nous pouvons créer un tableau vide *numpy* de la taille (forme) correspondant au nombre de lignes et de colonnes valides dans chaque fichier:
  ```
  data_array = np.empty((rows, cols), dtype=np.float32)
  ```
* * Pourquoi ne pas utiliser directement `np.empty((31, 12)` même si la forme de tous les fichiers est la même?<br> Nous voulons écrire une fonction généralement valide et les deux lignes pour calculer le nombre valide de lignes et de colonnes font la tâche de généralisation.*
* Ensuite, nous devons analyser les valeurs de chaque ligne et les ajouter au vide jusqu'à présent `data_array`. Par conséquent, nous avons divisé `f_content` en ses lignes avec `split("\n)` et utiliser une boucle *for*: `for iteration, line in enumerate(f_content.strip("\n").split("\n"):`. <br> Créer une liste vide pour stocker les données en ligne `line_data = []`. <br> Dans une autre boucle *for*, strip et split la ligne par l'utilisateur-défini `delimiter` (rappel: nous utiliserons `delimiter=";"`) `for e in line.strip(delimiter).split(delimiter):`. Dans la boucle *e-for*, `try:` pour ajouter `e` comme numéro *float* `line_data.append(np.float32(e)` et utiliser `except ValueError:` à `line_data.append(np.nan)` (i.e., ajouter une valeur non-a-numéro dont nous aurons besoin parce que tous les mois n'ont pas 31 jours).<br> Mettre fin à la boucle *e-for* en faisant appel à la boucle `for iteration, line in ...` et en ajoutant le tableau `line_data` *list* comme tableau *numpy* à `data_array`: `data_array[iteration] = np.array(line_data)`
* Retour dans l'énoncé `with open(file, ...` (utiliser le niveau d'indentation correct!), mise à jour `file_content_dict` avec le found `dict_key` et le `data_array` du `file as f`: `file_content_dict.update({dict_key: data_array})`

* Retour au niveau de la fonction (`def read_data(...):` - attention à l'indentation correcte!), `return file_content_dict`
...
* Pour obtenir des statistiques par colonne (mensuelle), transposez le tableau d'entrée :
```
...
```
* Retour `monthly_stats` comme `numpy.array`:
```
...
* Calculez les différences mensuelles de stockage ($In_{m}$ - $Out_{m}$), par exemple dans une boucle *for* sur le dictionnaire `in_vol_series` :
