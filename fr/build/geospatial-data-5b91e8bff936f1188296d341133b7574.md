...
* lignes (Poly) composées de lignes définies par les points de départ et les paramètres.
...
* un fichier `.shx`, où les indices des géométries sont stockés,
* un fichier `.prj` qui stocke la projection,
...
* **Pyramides de carrelage (par exemple, {term}`GeoTIFF`-style ou Modèle numérique de terrain (MNT)),
