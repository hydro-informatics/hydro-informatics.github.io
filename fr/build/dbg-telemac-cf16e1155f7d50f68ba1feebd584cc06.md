...
: Trop de couches verticales peuvent entraîner des éléments de maille 3d très minces qui provoquent localement des flux supercritiques. Ainsi, envisagez de réduire le {ref}`NUMBER OF HORIZONTAL LEVELS <tm3d-slf-vertical>` dans le fichier de direction pour satisfaire l'état {term}`CFL`.
