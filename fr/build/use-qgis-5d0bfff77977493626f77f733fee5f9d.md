...
* Dans le menu supérieur aller à **Projet** > **Propriétés**
...
* Récupérer les coordonnées de points dans n'importe quel format Système de coordonnées
...
:name: qgis-plugins
* Le plugin *Crayfish* pour la post-traitement de la sortie numérique du modèle.
...
- `*.geo`, `*.stl`, `*.msh`
- Éléments finis 3D ([Netgen](http://ngsolve.org/) et [Mmg3d](https://www.mmgtools.org/)), compatibilité avec {ref}`salome-install`
...
- S'adapte à plusieurs algorithmes de maillage (entre autres : {cite:t}`shewchuk1996`)
* - DEMto3D
...
* Activer le snapping pour éviter les écarts entre la plaine inondable et les polygones du canal principal
* Activer la barre d'outils ** : **Voir** > **Barres d'outils** > **Barre d'outils d'échantillonnage**
* Activer le snapping à partir de **Snapping toolbar** > **Enable Snapping** et **Éviter le chevauchement en polygone**
* Pour dessiner un polygone, allez à la barre d'outils ** **Ajouter une fonctionnalité en polygone** avec l'option **Digitiser avec l'option Segment** activée
* Commencez à dessiner en cliquant sur la carte (clic droit final Polygon)
* Dessiner un polygone du canal principal et après la finalisation :
...
* Si vous avez fait une erreur de dessin, utilisez soit la table *Attribute* pour sélectionner et supprimer des polygones entiers, soit utilisez l'outil vertex <img src="../img/qgis/sym-vertex-tool.png"> de la barre de menu.
...
* ** Champ à utiliser pour une valeur de gravure** : `ManningN`
...
* ... défiler vers le bas ...
* **Étendue des sorties** : cliquez sur le bouton **...** > **Calculer à partir du calque** > `FlowAreas`
...
* ** Numéro de bande** : la bande de raster pour l'insertion de la valeur polygonale (c.-à-d. champ dans la table d'attributs); quelques notes :
* cet algorithme va tourner les décimales en entiers (voir vidéo ci-dessous)
* alternativement, rechercher *Raster pixels vers polygones* dans la *Processing Toolbox*, mais cela créera un nombre excessif de polygones
...
* Dans le format **Raster Calculator Expression** type de cadre **1**, puis cliquez sur le bouton **/** (**Operators** frame), puis sélectionnez **roughness@1** dans le format **Raster Bands** frame.
* Le cadre de calculatrice **Raster Expression** doit maintenant contenir : `1 / "roughness@1"`, où le signe `@` se réfère au numéro de bande `1`.
* Cliquez sur **OK** pour lancer *Raster Calculator*.
* Après un calcul réussi, modifiez en option la symlogie de la nouvelle couche (*collant de rouille*).
...
* Dans la fenêtre **Traduisez le format**, effectuez les réglages suivants :
* **Couche d'entrée** = le raster (par exemple, un {term}`DEM`) à convertir
* ** Paramètres avancés** cadre > **Type de données de sortie** > sélectionnez **Float32** (correspond à une seule précision dans les modèles numériques)
...
* Définissez un titre **Layout** (p. ex.
* Dans la nouvelle (* exercice-exécution*) Disposition:
* Aller à **Ajouter un élément** > **Ajouter une carte**.
* Dessinez un rectangle qui contiendra la carte.
* **Ajouter un point** > **Ajouter une barre d'échelle**
* Pour contrôler les échelles et les unités indiquées dans la barre d'échelle :
* Dans **Items** panel, surlignez `<Scalebar>` et trouvez l'onglet **Item Properties** ci-dessous.
* Dans l'onglet Propriétés de l'article** modifier les unités à votre convenance.
* **Ajouter un point** > **Ajouter une légende**
* Pour contrôler les éléments de la légende :
* Dans **Items** panel, surlignez `<Legend>` et trouvez l'onglet **Item Properties** ci-dessous.
* Dans l'onglet **Propriétés de l'article**, trouver **Articles législatifs** > désactiver **Mise à jour automatique** > **supprimer** *OpenStreetMap* et *Google Satellite*.
...
* Facultatif, pour les graphiques SVG-vector: **Layout** > **Exporter en tant que SVG**
