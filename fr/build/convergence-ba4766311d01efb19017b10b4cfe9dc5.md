...
Si le fichier de direction active `VARIABLE TIME-STEP : YES` (une étape adaptative contrôlée par Nombre de Courant), le nombre d'étapes de temps n'est pas connu a priori et `NUMBER OF TIME STEPS` ne lie pas l'heure simulée; l'exécution est alors plafonnée par `DURATION`, et le temps de convergence identifié ci-dessus doit être interprété en secondes de simulation plutôt qu'en étapes de temps.
```
