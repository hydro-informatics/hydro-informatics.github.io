...
* Pour les essais parallèles, utilisez la commande `decomposePar`, comme cela a été fait pendant le processus de maillage, pour décomposer la géométrie en géométries individuelles pour chaque [MPI (Message Passing Interface, un standard pour le calcul parallèle)](https://www.mpi-forum.org/) process.
...
* Lancer la simulation en tapant ce qui suit dans la fenêtre du terminal :

* Dans le cas d'exécutions parallèles (suppléter "x" avec le nombre de carottes):
...
* La première ligne indique le débit moyen et maximal {term}`CFL` condition.
* La ligne 2 montre plutôt l'interface {term}`CFL` condition, qui est plus restrictive que la précédente et devrait être strictement inférieure à 1 lors de la résolution des flux multiphasés.
* "MULES: Correction alpha.water" dépend de la valeur définie à *nAlphaCorr* définie dans le fichier *fvSolution*.
* Les lignes 7 et 9 se réfèrent au *nAlphaSubCycles*, qui a été fixé à 1, ce qui signifie une seule boucle.
* Les lignes 11, 13 et 15 font référence aux trois correcteurs de pression et aucune correction non orthogonale n'est présente.
