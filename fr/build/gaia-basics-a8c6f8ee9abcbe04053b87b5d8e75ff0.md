...
Rappelez-vous la structure de l'hydrodynamique [boundaries.cli](https://github.com/hydro-informatics/telemac/raw/main/gaia2d-tutorial/boundaries.cli) fichier, qui a 13 valeurs (c.-à-d., colonnes) par ligne (ligne), qui sont séparés par un `space` et c'est ainsi que la tête de fichier ressemble (pour les limites de mur fermé `2`-type):
...
* (2) `LIUBOR`: code limite pour la composante vitesse $u$
* (3) `LIVBOR`: code limite pour le composant $v$ vitesse
* (4) `HBOR`: valeur de la profondeur de débit (en m) *-non utilisé dans ce tutoriel*
...
* (10) `EBOR`: **Élévation du lit de rivière spécifique à Gaia** (m a.s.l.)
* (11) `CBOR`: **Équilibre spécifique à Gaia** concentration de sédiments en suspension (en g$\cdot$L$^{-1}$) pour la modélisation de charge en suspension (en savoir plus dans le {ref}`section on suspended load <gaia-sl>`)
* (12) Numéro de point
* (13) Numéro d'ordre du point de frontière

**La structure du fichier des conditions limites varie entre les modèles Gaia couplés à Telemac2d et à Telemac3d. Ainsi, lorsque vous passez de 2d à 3d modèles, créez de nouveaux fichiers de conditions limites.**
...
* `1` définit les limites de l'onde incidente ou de Thompson
* `2` définit les limites du mur
...
Pour ce tutoriel, commencez par **créer une copie du fichier hydrodynamique limits.cli**, nommez-le **borderies-gaia.cli** et **remplacez la colonne (8)** (c.-à-d. tous `LIHBOR` occurrences) avec les valeurs suivantes:
...
* `NCO` définit **n**on-**co**sédiment hesif
...
* **CONCENTRATION MUD PAR LAYER** définit les concentrations massiques dans chaque couche (jusqu'à 20) pour le modèle de consolidation dans ($g\cdot L^{-1}$).
* ** PARTENAIRES PARTENIADES CONSTANT** définit les flux d'érosion à (kg$\cdot$m$^{-2}\cdot$s$^{-1}$) pour chaque couche (jusqu'à 20 couches).
* **LAYERS CRITIQUE ÉROSION STRESS DU MUD** définit la contrainte critique de cisaillement d'érosion dans (N$\cdot$m$^{-2}$) pour chaque couche (jusqu'à 20 couches).
* **LAYERS MUD CONCENTRATION** définit les concentrations de masse dans (g$\cdot$L$^{-1}$) pour chaque couche (jusqu'à 20 couches) du modèle de consolidation.
...
* La couche **disturbance englobe la progression des vagues de sable** sous la forme de **scour et de remplissage** sur une échelle **event**. Son épaisseur est de 0,5 fois la hauteur des dunes (ou ondulations).

Bien que Gaia n'accepte qu'un mot-clé **ACTIVE LAYER THICNESS**, il peut se référer à la couche *active* comme un multiple de la taille représentative du grain, ou lorsque les sédiments fins sont présents ($\geq$ 20%), à la couche *disturbance* avec une épaisseur de 0,5 fois la hauteur de la dune. Lorsque le lit de rivière est composé de galets et de gravier avec une petite part de sédiments fins (environ entre 1 et 20 %), l'épaisseur de la couche active doit être généreusement supposée avec un multiple (2-3 fois) de la taille des galets.
