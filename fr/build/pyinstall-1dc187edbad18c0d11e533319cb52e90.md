...
* **`pip` + `venv`** pour Linux/macOS (alternative; utiliser de préférence `mamba`),
* des améliorations récentes dans les roues binaires pour GDAL/Fiona/Shapely (pip sur Windows est enfin sans douleur!), et
...
* Utilisez `pip` seulement lorsque vous * devez* garder l'environnement minimal.

(conda-quick)=
### Guide rapide (Anaconda / Miniforge)

1. Installer **Miniforge 4** (plus léger qu'Anaconda, par défaut pour conda-forge) -- se référer aux instructions d'installation sur le [site Web](https://conda-forge.org/download/).
2. Obtenez les flusstools [environment.yml](https://raw.githubusercontent.com/Ecohydraulics/flusstools-pckg/refs/heads/main/environment.yml) et utilisez-les pour créer le conda `flussenv` (prend quelques minutes):
    ```bash
...
* Commencez par mettre à niveau les **roots** (par exemple, `pip install -U jupyterlab jupyter-book nbclient nbconvert`) et réessayer.
* Si l'erreur désigne une broche spécifique dans votre `requirements.txt`, détendez-vous **juste celle** (ou supprimez la ligne) et recourez.
* Si vous mélangez les installables/VCS, mettez-les à niveau individuellement : `pip install -U git+https://...`.
* Pire cas: retournez à votre instantané:
...
* Utilisez **`python -m venv`** (`pipx` pour les outils CLI) à moins qu'un paquet *requiert* les bibliothèques C/C++ que votre OS ne peut pas satisfaire – puis atteindre **conda-forge**.
* Les utilisateurs de Windows ne *need* conda pour GDAL, mais conda reste le chemin le plus facile pour une pile de données géospatiales.
* Épinglez les versions exactes pour les projets d'archives; utilisez `>=` pins pour le code de recherche vivant.
* Ne jamais installer des paquets avec `sudo pip`; toujours travailler dans un environnement isolé.
