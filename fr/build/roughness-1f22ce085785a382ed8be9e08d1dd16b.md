...
* un drone moderne (pas trop cher)
...
* **Nom du fichier**: appuyez sur **...**, naviguez jusqu'au dossier du projet, et appuyez sur `friction-polygons`.
* **Encodage du fichier**: garder par défaut (les fichiers échantillons utilisent `UTF-8`).
* **Type de géométrie**: `Polygon`
* Au-dessous des dimensions supplémentaires* (non obligatoires), trouver et cliquer sur le bouton **Système de coordonnées** pour sélectionner le système de coordonnées de projet (exemple de fichiers : `EPSG:32633`).
...
* ** Nombre de points ou densité**: `0.25` - utiliser une valeur plus petite/plus grande pour les mailles fines/courbées, mais méfiez-vous des très grandes tailles de fichiers
* **Distance minimale entre les points**: `5.0` - utiliser une valeur plus petite/plus grande pour les mailles fines/courbées
* **Points de rando** : cliquez sur **...** et définissez un nom de fichier de shapefile de point cible, comme `friction-pts.shp` à stocker dans le dossier du projet.
* Cliquez sur **Run** pour créer les points shapefile. Ce processus peut prendre un certain temps en fonction de la densité de points définie.
...
Le transfert des attributs `dMean` et/ou `fricID` des polygones aux points est essentiellement une opération d'interpolation dans laquelle QGIS regarde chaque point et lui attribue les attributs `dMean` et/ou `fricID` du polygone le plus proche. À cette fin, cliquez sur le menu supérieur **Vector** > **Outils de gestion des données** > **Join Attributs by Location** (voir {numref}`Fig. %s <fric-data-mgmt-join-attributes>`).
...
* ** Caractéristiques (prédice géométrique)** : cocher la case `are within`, désélectionner tous les autres
...
* **Couche jointe** : cliquez sur le bouton **...** > **Enregistrer dans le fichier** > naviguer dans le dossier **projet** > **entrer un nom de fichier, tel que `friction-pts-at`**
* Cliquez sur **Run**.
...
1. supprimer les colonnes inutiles, comme le champ `id`, et éventuellement aussi le champ `fricID` (cette vitrine n'utilisera que la colonne `dMean`),
1. ouvrez la calculatrice **Field**, que nous utiliserons dans la prochaine étape pour calculer les valeurs de rugosité Strickler.
...
* Entrez une formule en sélectionnant la valeur x de la géométrie dans la boîte de défilement ou en entrant directement dans le champ **Expression** :
...
* **Nom du fichier** : cliquez sur **...**, naviguez dans le dossier du projet et entrez `friction-pts.xyz` pour le nom du fichier ( appuyez sur **Save**).
...
* **Sélectionnez** tous les champs ** pertinents**, c'est-à-dire, dans la vitrine, au moins `k_st`. Les champs optionnels `x_coord` et `y_coord` ne sont nécessaires que si la géométrie n'est pas exportée (pour une raison quelconque).
...
* Réglez le **GEOMETRY** à `AS_XY`
* établir le **SEPARATEUR** à `TAB`
* **Décocher** le fichier **Ajouter le fichier enregistré à la carte** en bas de la fenêtre.
* **Gardez tous les autres par défaut** et cliquez sur **OK**.
...
* mettre en valeur `qgismesh.slf`, et appuyez sur **Ouvrir**.
...
* ** Valeur du nœud par défaut**: `30` (dans cet exemple) pour une valeur par défaut (Strickler) à utiliser lorsque aucun point de friction xyz ne se trouve à proximité d'un noeud maillé
...
* Mettre en évidence la nouvelle variable de maillage `BOTTOM FRICTION` (ou `FRIC_ID`) (en **Données**)
* Allez à **Outils** > **Objet Map...** (menu supérieur à {numref}`Fig. %s <bk-map-2d-interpolator>`)
...
* Dans **Espace de travail** > **Données** > **qgismesh**, **droit-clic** sur la nouvelle variable **FRICTION BOTTOM** > **Propriétés**.
* Dans les propriétés, aller à l'onglet **ColourScale**, et utiliser, par exemple, une échelle *Linear* avec `10` *Levels*, un *Min* de `22`, et un *Interval* de `1.8`. Le minimum et l'intervalle exemplaires sont de bons choix pour la vitrine, mais d'autres paramètres pourraient être préférables pour d'autres applications (p. ex., préférez *Min* de `0` lors de l'utilisation de Manning $n_m$).
...
* ouvert *boundarys.cli* dans un {ref}`text editor <npp>`
* trouver la limite en amont `5 5 5` (préciser Q et H) et la remplacer par `4 5 5` (préciser Q seulement)
* enregistrer et fermer * frontières.cli*
* pour plus d'informations, consultez le chapitre sur les projecteurs {ref}`boundary conditions <tm-foc-bc>`
* [Télécharger les limites adaptées.cli here](https://github.com/hydro-informatics/telemac/raw/main/friction/boundaries.cli).
...
* la valeur corrigée [boundarys.cli](https://github.com/hydro-informatics/telemac/raw/main/friction/boundaries.cli)
...
L'annexe E du [Manuel de Telemac2d](https://gitlab.pam-retd.fr/otm/telemac-mascaret/-/blob/v9.0.0/documentation/telemac2d/user/telemac2d_user_9.0.pdf) fournit des explications supplémentaires sur les paramètres tabulaires (p. ex., `typeB`, `rB`, ou `nDefB`), et les lois de friction (p. ex., STRI, NIKU) sont expliquées plus en détail dans ce livre électronique dans le stable-2d {ref}`section on friction <tm2d-friction>`.
```
