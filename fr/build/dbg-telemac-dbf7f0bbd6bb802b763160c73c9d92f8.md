...
Depuis sa création, TELEMAC est devenu un outil robuste et fiable pour la modélisation numérique des flux de surface. Pourtant, il y a quelques petits défis et cette page toujours croissante apporte quelques réponses.
...
Lorsqu'un Telemac fonctionne en parallèle (en utilisant plusieurs cœurs) s'écrase, ou si vous avez besoin de recombiner des fichiers de sortie partiels de chaque sous-domaine dans un seul fichier, les outils de post-traitement Telemacs offrent des options pour fusionner (ou « pointer ») les résultats de submesh. Deux outils principaux sont disponibles :

1. **GRETEL (l ' outil dédié à la fusion)**
- GRETEL reprend les fichiers de résultats partiels (sous-domaine) de chaque noyau et les fusionne en un seul fichier SELAFIN/MED pour l'ensemble du domaine.
...
- PARTEL est généralement utilisé pour partitionner les maillages pour les parcours parallèles, mais il a aussi une capacité de fusion. Certaines versions ou workflows utilisent PARTEL pour partitionner et fusionner.
...
3. Une fois GRETEL terminé, il produit un fichier ** unique 3D SELAFIN** (ou MED) avec tous les sous-domaines réassemblés. Vous pouvez alors visualiser ou poursuivre le traitement de ce fichier fusionné dans votre environnement de post-traitement préféré (p. ex. Blue Kenue, Paraview, QGIS).
...
* Préférez `:` sur `=`
* Placez tous les fichiers modèles dans le même dossier et ** utilisez uniquement les noms de fichiers** sans les répertoires de fichiers.
...
* * *Connectivité des éléments?* qui signifie <br>Anglais: **Connectivité d'élément?**
+ Option `1`: Nodal
+ Option `2`: Descending
* *Il y a 1 courrier(s) de type local dans ce fichier. Lequel voulez-vous lire (0 pour tous=1=2=3=3=?* qui signifie <br>English: **Il y a 1 maille(s) locale(s) dans ce fichier. Lequel voulez-vous lire (0 pour tous ou pour tous ?**
+ Option `0`: Tout lire
...
Ainsi, ces deux limites doivent être référencées dans le fichier de direction (`*.cas`) comme suit pour prescrire un débit `Q` (p. ex., 10 m$^3$/s) en amont et une profondeur `H` (p. ex., 0,75 m) en aval :
...
* Créez manuellement des copies de bibliothèques plus récentes avec des noms de versions plus anciennes. Par exemple,
+ Dans la 4ème ligne après avoir lancé `./salome`, `Kernel/Session`
...
+ Créer une copie de la bibliothèque plus récente et renommer la copie comme nécessaire par SALOME; par exemple, appuyez sur

```
sudo cp /usr/lib/x86_64-linux-gnu/libSOMETHING.so.40 usr/lib/x86_64-linux-gnu/libSOMETHING.so.20
```

+ Probablement, les fichiers suivants doivent être copiés :
...
* Supprimez la version interne de SALOME-HYDRO de *Qt* :
+ Copier
...
* Ouvrez le fichier `~/.profile` (par exemple, utilisez le navigateur de fichiers, allez dans le dossier `Home` et appuyez sur `CTRL` + `H` pour basculer en regardant les fichiers cachés) et ajoutez au bas du fichier :
...
* Enregistrer et fermer `.profile` et redémarrer (ou simplement re-connecter).
