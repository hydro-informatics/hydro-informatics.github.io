...
* Faites un nouveau clic droit sur `/dev/sda3` et cliquez sur *Resize/Move*.
+ Dans la case *Espace libre qui précède (MiB):* saisissez une taille raisonnable pour libérer de l'espace disque pour la partition racine (par exemple `2000`).
...
* Faites un clic droit sur la partition racine (`/dev/sda2`) et cliquez sur *Redimensionner/Move*.
...
* Après le repartitionnement réussi terminé, faites un clic droit sur la partition après la partition racine (`/dev/sda3`) et assurez-vous qu'elle est à nouveau en mode *Swapoff*. Si ce n'est pas le cas (c.-à-d. que vous ne trouvez pas *Swapoff* dans le menu contextuel et que seul *Swapon* est visible), cliquez sur *Swapon*.
...
1. Télécharger et enregistrer la clé WineHQ dans un nouvel emplacement: Exécutez la commande suivante pour télécharger la clé, ma chère, et enregistrez-la dans l'emplacement recommandé (par exemple, `/usr/share/keyrings/winehq-archive.key`):
...
3. Actualiser les listes de colis :
...
* Téléchargez les LASTools (ZIP) de http://rapidlasso.com/LAStools et extraitz le fichier ZIP
...
* dans cet exemple, le pilote le plus récent est `nvidia-driver-515` (avec `nvidia-dkms-515`), que nous prenons note pour les installer deux étapes plus tard
...
2. Commencez un nouveau poject en cliquant sur **Projet de données**.
3. **Ajouter des fichiers** avec un clic sur l'icône plus (habituellement en haut à gauche) et sélectionner le dossier où le fichier IMG est monté (`/media/user/appname`).
4. Cliquez sur **Burn**, choisissez un répertoire de sortie et changez en option le nom de l'image de sortie.
...
2. ** Lancez une coquille Darling et lancez l'application :**
...
1. **Vérifier les scripts de lancement ou la documentation :**
