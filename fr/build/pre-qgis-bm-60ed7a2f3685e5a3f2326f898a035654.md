...
* [**Télécharger l'exemple Modèle numérique de terrain (MNT) GeoTIFF**](https://github.com/hydro-informatics/materials-bm/raw/main/rasters/dem.tif) et enregistrez-le dans le même dossier (`/Project Home/` ou un sous-répertoire) que le projet ci-dessus **qgz**.
* Ajouter le Modèle numérique de terrain (MNT) téléchargé comme nouveau calque raster dans *QGIS*:
* Dans *QGIS*' **Browser** panneau trouver le répertoire **Project Home** où vous avez téléchargé le Modèle numérique de terrain (MNT) *tif*.
* Faites glisser le Modèle numérique de terrain (MNT) *tif* du dossier **Project Home** dans le panneau QGIS **Layer**.
...
* Voir les lignes rouges à {numref}`Fig. %s <breaklines>`**.
* **Breaklines de la rive gauche (LB) et de la rive droite (RB)**:
* Dessinez des lignes le long du canal principal mouillé indiqué dans la carte de base des images satellitaires.
...
* Voir les lignes **yellow-orange dans {numref}`Fig. %s <breaklines>`** (notez la délimitation des petits affluents en haut à gauche sur la rive gauche et en bas à droite sur la rive droite).
...
* Assurez-vous que la ligne se termine parfaitement avec les lignes de rupture du chenal principal avant la création (lignes rigides); ainsi, les lignes de rupture du chenal principal et les lignes de rupture du banc de gravier doivent enfermer les rives de gravier sans écart entre les lignes.
...
* Pour le champ **LineType**, utilisez des valeurs de texte telles que **hardline sss** (ou autre chose - l'exemple se réfère au mot allemand <u>S</u>chütt<u>s</u>tein<u>s</u>chwelle).
* Voir les lignes bleues dans {numref}`Fig. %s <breaklines>`**.
* Facultatif : **Breakline d'un banc de sable** :
* Trouvez le dépôt de sable dans le coin supérieur gauche de {numref}`Fig. %s <breaklines>` sur la carte de base de l'imagerie satellite et délimitez-le en dessinant une ligne bien courbe.
* Assurez-vous que la ligne se termine parfaitement avec les lignes de rupture du canal principal et embrassez une zone fermée sans aucun écart entre les lignes.
* Pour le champ **LineType**, utilisez des valeurs de texte telles que **hardline sand**.
* Se reporter à la ligne **pourpre** dans le coin supérieur gauche **à {numref}`Fig. %s <breaklines>`**.
...
* Dessiner une limite d'entrée (voir aussi {numref}`Fig. %s <inflow-boundary>`):
...
* Commencez à tracer une ligne sur la rive gauche (côté gauche de la figure ci-dessous) et a déplacé vers l'est (c.-à-d. vers la droite) pour faire sept points de plus à travers la rivière.
* Le **septième point** doit ** coïncide** avec la fin de la ligne de démarcation de la rive droite**.
* Ainsi, l'écoulement en amont vient du côté droit de la ligne limite d'écoulement (c.-à-d., la direction d'écoulement en amont sera `right` pour le modèle numérique).
...
* Pour ** corriger les erreurs de dessin** utiliser le **Vertex Tool** <img src="../img/qgis/sym-vertex-tool.png">.
...
* Commencez à tracer une ligne sur la rive gauche (en haut de la figure ci-dessous) et déplacez-vous vers le sud-ouest (c.-à-d. vers le bas) pour faire sept points de plus à travers la rivière.
* Le **septième point** doit ** coïncide** avec la fin de la ligne de démarcation de la rive droite**.
* Ainsi, le flux en amont vient du côté droit de la ligne de limite de sortie (c.-à-d., la direction de flux en amont sera `right` pour le modèle numérique).
...
* Définissez le nom du fichier** comme **region-points.shp** (ou similaire)
* S'assurer que le type **Géométrie** est **Point**
...
* **max area** = **Nombre de décisions** (**longueur** = 10, **précision** = 3)
...
* **Couche de marqueur de la région** = **régions-points** (voir {ref}`regions`).
...
* Dans la couche **Mesh à interpoler** frame, sélectionnez **prepro-tutorial quality-mesh**.
...
* Sélectionner **dem.tif** GeoTIFF (voir {ref}`get-dem` section) comme **Couche de grille**.
* Dans le cadre **Output** cliquez sur le bouton **Browse** pour définir un nom de maillage de sortie dans le répertoire `/Project Home/`, par exemple, **prepro-tutorial quality-mesh-interp.2dm**
