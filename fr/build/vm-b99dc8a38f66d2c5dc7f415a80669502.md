...
* Utilisation meilleure et plus efficace des ressources matérielles existantes.
...
* Téléchargez la dernière version de Debian Linux (ou l'un de ses spin-offs - voir le point de puce suivant) installateur net (c'est la solution **recommandée** **pour** travaillant avec **TELEMAC**):
+ La version stable actuelle est **Debian 13** (nom de code *trixie*).
+ Trouvez la section CD et cliquez sur la version [*amd64*](https://cdimage.debian.org/debian-cd/current/amd64/iso-cd/)
...
+ Si le lien ci-dessus ne fonctionne pas, visitez [debian.org](https://www.debian.org/), trouvez la section *Obtenir Debian* (ne pas cliquer sur celle du menu supérieur), cliquez sur *CD/USB ISO images* et cliquez sur [Télécharger des images CD/DVD en utilisant HTTP](https://www.debian.org/CD/http-ftp/).
...
+ [*Lubuntu*](https://lubuntu.me/downloads/) est une dérivée rapide et légère de *Ubuntu* (l'actuel LTS est *Ubuntu 26.04*).
+ [*Linux Mint*](https://www.linuxmint.com) est une autre distribution efficace *Ubuntu* qui suit la version la plus récente *Ubuntu LTS* (par exemple, *Linux Mint 22.x* "Wilma"/"Xia"/"Zara"/"Zena" est construit sur *Ubuntu 24.04 LTS*).
...
* Définissez en option un nom d'hôte (par exemple, debian-vm) et un nom de domaine (par exemple, debian-net).
...
* Passez la question d'information de proxy (cliquez simplement sur *Continuer*).
...
* Sur l'hôte original:
* Éteignez le VM que vous voulez réutiliser.
* Dans VirtualBox allez à **Fichier** > **Appliance d'exportation...**.
* Suivez les instructions pour exporter le VM vers un fichier `*.ova` (le processus **export peut prendre 10-15 minutes**).
* Sur le nouvel hôte :
...
* une image de disque virtuel, comme `.vdi`, `.vmdk`, ou `.vhd`
...
* paramètres d'adaptateur réseau
...
* Affecter les ressources RAM et CPU.
...
* ** Paramètres d'affichage** : Régler la résolution et l'échelle de l'écran si nécessaire.
...
* **RAM**: Attribuez suffisamment de mémoire pour l'OS invité et les applications, mais laissez suffisamment de RAM pour le système hôte.
...
* **Réseau**: Utilisez NAT pour un accès simple à Internet, ou réseau ponté si la VM doit être visible sur le réseau local.
* **Snapshots** : Créez un instantané avant les mises à jour majeures ou les modifications de configuration.
...
* Cela peut arriver lorsque le *View* a été réglé sur le mode *Scaled*.
* Pour basculer le mode de vue et rendre la barre de menu visible, appuyez sur la touche DROITE `CTRL` (`Host`) + la `C` sur votre clavier, tout en étant dans la vue système hôte.
```
...
* Si vous choisissez une résolution trop haute, la fenêtre *VirtualBox VM* va tourner en noir et revenir à la résolution d'origine après 15-30 secondes.
...
* À un endroit de votre convenance, créez un nouveau dossier sur le système hôte (par exemple, *Windows 11*) et appelez-le partagé (par exemple, `C:\Users\USER\documents\shared\`).
...
* * Chemin du dossier: * Sélectionnez le nouveau dossier `...\shared`
* Cochez la case *Enable Auto-mount*
* Cochez la case *Faites Permanent*
* Cliquez sur OK sur les deux fenêtres contextuelles.
...
2. Créer une nouvelle bouteille: Ouvrir **Boucles** > Cliquez sur **+ Nouvelle bouteille** > Choisir :
...
* ** Polices principales** : `allfonts`
* **.NET framework**: `dotnet472` (pour HEC-RAS - l'installation peut prendre un certain temps)
* **C++ 2013 et 2015-2022 Redistribuables (x64)**: `vcredist2022` et `vcredist2012`

4. Retournez à la bouteille et ouvrez les **Paramètres** de la bouteille:

* Dans la section **Components**, assurez-vous que le Runner (`soda`), DXVK, VKD3D, et DXVK NVAPI sont activés; LatencyFleX peut être activé, mais pas besoin pour elle; si les choses sont **buggy** plus tard, essayez **désactiver DXVK** (et donc VKD3D, aussi)
* Dans la section **Affichage**, activer *Discrete Graphics*
* Dans la section **Affichage** > Paramètres avancés, désactiver *Bureau virtuel* (pour la stabilité); seulement activer *Capture de souris plein écran*, *Chauffe de souris* et *Décorations du gestionnaire de fenêtres*; vous pouvez changer le *Reender* -- ce paramètre n'a pas eu d'influence dans nos tests mais GDI est dit pour effectuer le meilleur et le plus stable
* Dans la section **Performance**, tout désactiver
* Dans la section **Compatibilité**, assurez-vous que *Windows 10* dans l'ensemble (11 était toujours buggy pendant les tests) et que tout le reste est désactivé
* Dans la section **Snapshots**, tout désactiver

5. Télécharger les installateurs concernés:

* Pour BlueKenue, obtenez le dernier installateur décrit dans le [Telemac Forum](https://www.opentelemac.org/index.php/assistance/forum5/blue-kenue/12464-how-to-download-the-latest-blue-kenue-and-report-bugs-and-suggestions) et sauvegardez-le à `~/installers/WindowsSoftware/`
* Pour HEC-RAS, allez à la page HEC-RAS [téléchargez la page](https://www.hec.usace.army.mil/software/hec-ras/download.aspx) et téléchargez et installez `~/installers/WindowsSoftware/`; **note: dans notre test, nous n'avons réussi à exécuter HEC-RAS 5.0.7 (ou plus tôt) -- des versions plus récentes tôt ou plus tard ont causé des erreurs**

6. Installer le logiciel dans la bouteille

* Ouvrir la bouteille > **Run Exécutable**
...
* Terminer les installateurs

7. Logiciel de lancement:

* Après l'installation réussie, de nouveaux logiciels devraient se produire dans la section **Programmes**
* Si ce n'est pas le cas, faites défiler vers le bas jusqu'à la section **Outils**, élargissez **Outils à vin Legacy** et cliquez sur **Explorer** pour naviguer dans le système de bac à sable
````
````{tab-item} Lutris
...
3. Vérifier Vulkan fonctionne réellement.
...
* Les utilisateurs de Flatpak Steam doivent également installer ProtonUp-Qt via Flatpak comme indiqué ci-dessus.
...
* Version de commutation Proton : Propriétés > Compatibilité > choisir un autre Proton (Experimental, un majeur stable, ou Proton-GE).
* Vérifier les fichiers : Propriétés > Fichiers installés > Vérifier l'intégrité.
* Suppression du préfixe Proton du jeu (voir *Gérer les préfixes* ci-dessous) et relance.
...
4. Lancez-le depuis Steam comme tout autre titre.
...
3. Enregistrement Proton (capture une session de débogage dans le dossier de jeu ou le répertoire d'accueil):
...
4. Tiges d'environnement de proton commun (utiliser seulement si nécessaire):
* Forcer DirectX 11 via DXVK au lieu de DirectX 12:
...
* Désactiver Esync/Fsync si vous voyez des pendaisons bizarres :
...
* Pour *usine réinitialiser* un environnement Proton de jeu, fermer Steam, puis supprimer son dossier à `compatdata/<APPID>/`. Vous perdrez par jeu des données de registre Windows et de préfixe de vin (traitez-le comme une nouvelle installation Windows pour ce titre).
* Pour parcourir le préfixe rapidement : faites un clic droit sur le jeu > Propriétés > Fichiers installés > *Parcourir* et naviguer vers le haut vers `compatdata`.

**Réseau de dépannage rapide:**

1. Le jeu ne démarre pas après une mise à jour:
* Version Switch Proton.
* Vérifier les fichiers de jeu.
* Supprimer le préfixe `compatdata/<APPID>` et relancer.

2. Écran noir avec audio:
* Alt+Enter pour basculer plein écran.
...
* Paramètres de vapeur > Contrôleur > activer l'entrée de vapeur pour votre appareil.
...
* L'onglet *Applications* permet de définir la couche de compatibilité *Windows* à utiliser (par exemple *Windows 10*) et à définir les applications.
...
* Envisagez d'ajouter un *Desktop Icon* ou notez le répertoire d'installation (p. ex. `"C:\\Program Files (x86)\\CHC\\BlueKenue\\"`).
...
* Si une icône du bureau* a été créée pendant l'installation, allez sur *Desktop* et double-cliquez sur l'application (par exemple, *BlueKenue*)
