...
Pour une meilleure expérience d'apprentissage, le {ref}`glossary` aide à expliquer les termes {term}`Sediment transport`, (dimensionless) {term}`bedload <Bedload>` transport $\Phi_b$, {term}`dimensionless bed shear stress <Dimensionless bed shear stress>` $\tau_{x}$, et le {term}`Shields parameter` $\tau_{x,cr}$ (dans cet ordre).
```
...
où $\alpha$ est l'angle entre l'axe longitudinal ($x$) et le vecteur de transport de la charge de lit (voir aussi {term}`Exner equation`), $\beta$ est un facteur empirique de correction de l'intensité de la charge de lit de {cite:t}`koch1980`, et $z_{b}$ est l'élévation de la charge de lit de rivière.
