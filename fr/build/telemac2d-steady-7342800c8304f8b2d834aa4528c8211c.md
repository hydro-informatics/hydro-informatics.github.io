...
* Telemac v8p2r0 ou plus récent ({ref}`stand-alone installation <modular-install>`).
* {ref}`QGIS <qgis-install>` et le {ref}`PostTelemac plugin <tm-qgis-plugins>`.
* Debian Linux 10 (Buster) / Debian 11 installé sur une machine virtuelle (en savoir plus sur {ref}`software chapter <chpt-vm-linux>`).
```
...
* `cd` vers le répertoire d'installation (téléchargement) de Fudaa PrePro
* Démarrez l'interface utilisateur (demande java):
...
* `EQUATIONS : SAINT-VENANT FE` est le **default** qui fait que Telemac2d utilise une méthode d'éléments finis de Saint-Venant,
* `EQUATIONS : SAINT-VENANT FV` fait Telemac2d utiliser une méthode de volume fini Saint-Venant, et
...
* `11` (**default**) active la discrétisation triangulaire (linéaire) dans l'espace (c.-à-d. triangles 3 nœuds),
...
* `1` définit une méthode non-conservatrice de masse des caractéristiques (par défaut pour tous),
...
* `15` définit le plan ERIA de masse-conservateur qui fonctionne avec les plates-formes de marée.
...
* `1` permet de remonter avec un système de SUPP classique (recommandé lorsque la condition {term}`CFL` est inconnue), et
* `2` permet le remontage avec un schéma SUPG modifié, où le remontage est égal à la condition {term}`CFL` (recommandé lorsque la condition {term}`CFL` est petite).

Par défaut, `SUPG OPTION : 2;2;2;2`, où

* le premier élément de la liste se réfère à la vitesse du flux (par défaut `2`),
* la deuxième profondeur d'eau (par défaut `2` - défini à `0` quand `MATRIX STORAGE : 3`),
...
* tout `SCHEME FOR ADVECTION ...` est réglé à `1`, ou
* tout `SCHEME OPTION FOR ADVECTION OF ...` est fixé à `2`.
...
* `0` permet le plan {cite:t}`roe1981ars`,
* `1` est le **default** et permet le schéma cinétique {cite:p}`audusse2000`,
* `3` permet le plan {cite:t}`zokagoa2010` qui est incompatible avec les plates-formes de marée,
...
* `6` permet le flux moyen pondéré (WAF) {cite:p}`ata2012` plan pour lequel le parallélisme n'est pas actuellement mis en œuvre.
...
* `7` définit la méthode du RESiduel minimum généralisé* (**GMRES**), et
...
* `2` supprime les éléments plats de marée en utilisant une table de masque qui élimine toute contribution des éléments de maille concernés. Cette option peut affecter la conservation de masse du modèle.
* `3` ressemble à `1`, mais ajoute un terme de porosité à des éléments de maille semi-sec. Cela affecte la quantité d'eau dans le modèle, ce qui équivaut ici à la profondeur intégrale multipliée par la porosité. Un fichier utilisateur Fortran peut être utilisé pour modifier le terme porosité dans le sous-routine `USER_CORPOR`.
...
* L'établissement du seuil à, par exemple, `-0.1` fait que les profondeurs d'eau négatives plus grandes (p. ex. -0,05 m) que -0,1 mètres restent inchangées.
* `2` impose une limite de flux qui garantit strictement des profondeurs d'eau positives.
...
* `3` pour utiliser le stockage matriciel basé sur le bord (par défaut). Cette valeur par défaut est requise lorsque tout mot-clé **CHEME D'ADOPTION ...** est défini à `3`, `4`, `5`, `13`, `14` ou `15`, et lorsque tout mot-clé direct **SOLVER** est défini à `8`.
...
* `0` sans friction.
* `1` pour l'équation {cite:t}`haaland1983`, qui est une forme implicite de l'équation {cite:t}`colebrook1937` qui s'appuie sur le facteur de friction Darcy-Wesbach $f_D$. Cette loi comporte un degré élevé d'incertitude découlant de l'ensemble de données expérimentales de l'auteur original.
* `2` pour la rugosité {cite:t}`chezy_formula_1776` qui peut également être utilisée comme `3` et `4`.
* `3` pour {cite:t}`strickler_beitrage_1923` rugosité $k_{st}$ (lire davantage, par exemple, dans le {ref}` 1d hydraulics exercise <ex-1d-hydraulics>`), qui est l'inverse de $n_m$ (`4`).
* `4` pour {cite:t}`manning_transactions_1891` rugosité $n_m$ (en savoir plus, par exemple, sur le {ref}` 1d hydraulics exercise <ex-1d-hydraulics>`), qui est l'inverse de $k_{st}$ (`3`).
* `5` pour la loi {cite:t}`nikuradse_stromungsgesetze_1933` rugosité, qui devrait correspondre à 3 $\cdot D_{90}$ selon {cite:t}`vanrijn2019`.
...
* `7` pour l'équation {cite:t}`colebrook1937` qui calcule le facteur de friction Darcy-Weisbach $f_D$ pour les flux turbulents dans les tuyaux lisses.
...
* Dans **Processus > Boîte à outils**, exécuter **slf2med** (fournisseur Q4TS):
* **Input .slf**: `r2dsteady.slf`
* **Input .cli** (facultatif): votre fichier limite si vous le souhaitez
* **Output .med**: enregistrer `r2dsteady.med`
...
* Numériser une ou plusieurs lignes transversales à travers le canal (en amont, en aval, sections de contrôle, etc.).

2. Exporter les valeurs transversales du maillage vers le CSV
* Ouvrir **Processus > Boîte à outils** et exécuter **Exporter les valeurs de l'ensemble de données de section transversale sur les lignes du maillage**
* Configurer :
...
* toute variable de diagnostic que vous avez écrite aux résultats (si disponible)
* **Heure des données**:
* utiliser **Temps de toile actuel** pour les vérifications "snapshot", ou
* exécutez plusieurs fois pour des horodatages spécifiques qui vous intéressent.
* **Lignes pour l'exportation de données**: `control_sections`
...
* les vérifications de la santé de section par section (p. ex., les patrons de profondeur/vitesse où vous les attendez);
* détection de points chauds (épis non physiques près des limites, autour de gradients bathymétriques abrupts, etc.),
* "Est-ce qu'il est encore stable?" vérifie en comparant la même section à plusieurs étapes du temps.
...
* près des commandes hydrauliques,
* dans les zones où l'instabilité est probable (zones de chasse, fortes pentes, mouillage/sèchement du front).
2. Exporter des séries chronologiques du maillage au CSV:
* Ouvrir **Processing > Toolbox** et exécuter **Exporter les valeurs des séries chronologiques à partir de points d'un ensemble de données de maillage**
* Configurer :
* **Couche de maille d'entrée**: `r2dsteady`
...
* ** Points pour l'exportation de données**: `control_points`
* **Output**: enregistrer comme `*.csv`
...
* Est-ce que les noeuds peu profonds se retournent humide/sec irréalistement (question d'accord de mouillage/sèchement)?
...
* **Couche de maille d'entrée** : sélectionnez la couche de maille de résultats Telemac (`r2dsteady`)
...
* **Dataset time**: cliquez sur le symbole de flèche vers le haut/vers le bas pour défiler vers le bas et sélectionnez la dernière étape. Dans une simulation instable (c.-à-d. quasi stable), d'autres étapes de temps pourraient également être intéressantes.
...
* **Couche raster sortant** : cliquez sur **...** pour naviguer dans un dossier cible et entrer un nom pour le raster. Ici : `velocity-tmax.tif`.
...
* Inverser l'ordre des couches dans GIMP : **Layer** > **Stack** > **Reverser l'ordre des couches**.
* Enregistrer l'image comme GIF: **Fichier** > **Exporter comme...**.
* Sélectionnez un dossier pour enregistrer le fichier, dans le champ **Nom** entrer `[any-name].GIF`, et cliquez sur **Export**.
* Dans la fenêtre contextuelle activer **En animation** et **Loop forever** avec un délai recommandé entre les cadres de **100 millisecondes**. Gardez toutes les autres valeurs par défaut et cliquez sur **Export**.
...
* Dans le champ **Find** type `5 5 5`.
* Dans le **Remplacement avec** type de champ `4 5 5`.
* Cliquez sur **Replacer** jusqu'à ce que tous les types de bornes **en amont** soient modifiés.
