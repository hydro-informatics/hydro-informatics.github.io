...
Metashape a plusieurs workflows intégrés qui nous permettent de naviguer intuitivement dans la création de produits SfM basés sur l'imagerie. Nous utiliserons certains de ces flux de travail dans les éléments suivants pour dériver un {term}`DEM`.
```
...
* Réglez la précision** à **haut** pour un équilibre acceptable de résolution et de temps de calcul. Notez que toute augmentation supplémentaire de la précision donne lieu à un ordre de grandeur plus long de calculs.
...
* la limite **Tie point** définit un nombre minimum pour les points de liaison (c'est-à-dire les points que l'algorithme d'alignement identifie sur deux images voisines): assurez-vous de permettre l'identification d'au moins 1.000 à 5.000 points de liaison.
* utiliser l'option **activable d'exclusion des points stationnaires**, qui fait en sorte que les pixels ayant les mêmes caractéristiques (couleur) sur toutes les images (c.-à-d. qui sont stationnaires) soient exclus de l'identification d'alignement. Ainsi, par exemple, les taches de saleté sur l'objectif de la caméra seront exclues comme points clés d'alignement.
* Activez le modèle de montage de la caméra ** adapté** pour une distorsion de la caméra non corrigée.
...
* Choisissez **remplissement en profondeur** en fonction de la densité de végétation:
* Si non à très peu de végétation, sélectionnez **modéré / agressif**
* Si la végétation est présente, il est possible d'obtenir une bonne performance (qualité du nuage par rapport au temps de calcul) en choisissant l'option **mild**
* dans le cas de végétation très dense, le remplissage de profondeur peut être **désactivé**, bien que cela puisse conduire à **très long temps de calcul**
...
Le nuage point dense ou grossier peut être utilisé avec beaucoup d'autres workflows et la création d'un {term}`DEM` n'est qu'une option. Pour créer un Modèle numérique de terrain (MNT), allez à **Travail** > **Construire Modèle numérique de terrain (MNT)** et considérer les aspects suivants dans la fenêtre contextuelle de build Modèle numérique de terrain (MNT) (voir figure ci-dessous):
...
* Données sources:
* l'option **tie points** conduit à la plus basse résolution**.
