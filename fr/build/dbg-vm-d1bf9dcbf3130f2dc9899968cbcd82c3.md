...
* Entrez le menu de démarrage pendant le démarrage : Au tout début du système, appuyez généralement sur la touche `F2` ou `DEL`. Selon l'ordinateur, d'autres clés peuvent s'appliquer (par exemple, `F12` - regardez l'écran pour savoir comment accéder à `Setup`).
* Le gestionnaire de démarrage ouvre. Utilisez les touches fléchées pour naviguer à `Advanced settings`, appuyez sur Entrée et allez à `CPU Configuration`.
...
* Appuyez sur `Esc`, allez à `Exit` > `Save changes and exit` (ou appuyez sur la touche `F10`). Confirmez. Le système va redémarrer.
* Retour dans *Windows* ré-exécutez *VirtualBox* et démarrez un VM. Le message d'erreur ne devrait plus apparaître.
...
* Ouvrir `etc/apt/sources.list` et modifier la définition `buster`repositoire (exemple pour le serveur en Allemagne):
