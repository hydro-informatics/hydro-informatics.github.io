...
* **Compilateurs Intel**: `source pysource.intel.sh`
...
1. Le projet Système de coordonnées correspond au Système de coordonnées maillage (EPSG:25833)
2. La reprojection à la volée est activée si un projet différent est utilisé.
3. Le fichier géométrique d'origine a utilisé le système de référence de coordonnées correctes
...
* Dans le tableau **Layer**, sélectionnez **rGaia-steady2d** (ou *r2dsteady-gaia*).
...
* Dans la fenêtre **Export Animation**, allez à l'onglet **General** et définissez un nom de fichier de sortie en cliquant sur le bouton **...** (par exemple `velocity-video.avi`).
* Adaptation facultative des paramètres *Layout* et *Vidéo*.
...
- [ ] La direction de la charge de lit s'aligne sur la direction du flux principal (avec les écarts attendus dans les courbes)
- [ ] L'amplitude de l'évolution du lit est plausible (aucun trou de 10 m au débit de base)
- [ ] Le bilan massique des sédiments est fermé (vérifier la sortie du bilan massique de Gaia)
...
- [ ] Les concentrations se situent à l'intérieur d'intervalles physiquement raisonnables (généralement < 10 g/l pour les rivières)
...
* Si l'érosion est surestimée, augmenter ** PARAMÈTRES DE CLASSES**.
* Si l'érosion est sous-estimée, réduire ** PARAMÈTRES DE CLASSES**.
...
* Gamme typique: 1.0-2.0.

* Pour ajuster le profil de dépôt et d'érosion dans les courbes (riverbends), activez le mot-clé **CURRENTS SECONDAIRES** et modifiez la valeur **CURRENTS SECONDAIRES ALPHA COEFFICIENT** (cf. {ref}`Secondary Currents <gaia-secondary>`):
* Défaut : `1.0` (lit lisse)
* Pour les lits bruts : `0.75`
* Influe sur l'intensité du flux hélicoïdal et donc la redistribution latérale des sédiments.

* Le mot clé **HIDING FACTOR FORMULA** (pour les sédiments multiclasses) contrôle la façon dont les particules plus fines sont cachées par des particules plus grossières :
...
1. **Première** : Étalonnage de la magnitude globale du transport à l ' aide de parameters de CLASSES** ou de parameters de MPM**
2. **Deuxième** : Ajuster la distribution spatiale en utilisant **BETA** et **CURRENTS SECONDAIRES ALPHA COEFFICIENT**
3. **Troisième**: Fine-tune utilisant **RATIO ENTRE LA FRICTION DE LA PEAU ET LE DIAMÈTRE MÉNÉ**
4. **Dernier**: Ajustez les effets de cache/exposition pour les modèles multiclasses en utilisant **FACTEUR HIDING FORMULA**
...
- Augmentation des trajectoires de transport et amélioration des dépôts
...
- Diminution des taux d ' érosion
- Plage typique: 1.E-04 à 1.E-02 kg/(m2·s)

* **STRESS DE CRITIQUE DU MUD :
- Augmentation pour réduire l'érosion (seuil plus élevé)
- Diminution pour renforcer l'érosion (seuil inférieur)
- Variantes avec consolidation des sédiments; plage typique: 0,01-1,0 N/m2
...
- Des valeurs inférieures concentrent les panaches de sédiments
...
2. **Deuxième** : Ajuster **LAYERS STRESS CRITIQUE DE LA FEUILLE D'ÉROSION DU MUD** pour contrôler l'origine de l'érosion
3. **Troisième**: Fine-tune **CLASSES STRESS CRITIQUE POUR DÉPÔT MUD**
