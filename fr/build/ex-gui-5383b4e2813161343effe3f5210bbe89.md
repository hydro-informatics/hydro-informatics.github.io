...
* Ajouter trois arguments optionnels à la fonction `main()`:
  ...
  - `out_folder` pour activer la définition d'un répertoire de sortie défini par l'utilisateur pour le cahier des résultats de la charge de fond.
...
Pour exécuter le programme (calcul de la charge de fond), nous avons besoin d'un autre bouton, que nous voulons modifier plus tard pour communiquer que le programme a fonctionné avec succès. Ajouter le bouton d'exécution à `__init__`:
