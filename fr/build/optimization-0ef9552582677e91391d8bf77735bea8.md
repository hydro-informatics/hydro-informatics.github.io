...
* Désactiver `TIDAL FLATS`, même si la désactivation {ref}`tidal flats <tm2d-tidal>` ne peut pas être recommandée pour produire des modèles physiquement significatifs et stables.
...
* `IMPLICITATION FOR DIFFUSION` devrait être `1.` ou plus petit.
...
* Diminuer progressivement le délai.
* Diminuer la précision du résolveur (p. ex. de `1.E-8` à `1.E-6`).
