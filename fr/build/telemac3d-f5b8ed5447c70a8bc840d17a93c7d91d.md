...
* **Heure** :
  + `TIME STEP`: `1.0` définit l'étape de temps comme un multiple de périodes d'impression graphique/liste.<br>*Utilisez des étapes de temps suffisamment petites et suffisantes pour atteindre/augmenter la stabilité de calcul et augmenter l'efficacité de calcul.*
...
* `1` permet de remonter avec le schéma de SUPG classique (par défaut; recommandé lorsque la condition {term}`CFL` est inconnue), et
...
Une autre option pour accélérer la mise en place est de permettre la mise en masse pour la diffusion, la profondeur et/ou les caractéristiques faibles. La mise en masse entraîne une convergence plus rapide, mais elle introduit une dispersion artificielle des résultats, raison pour laquelle les développeurs de TELEMAC découragent la mise en masse. Le [steady3d.cas](https://github.com/hydro-informatics/telemac/raw/main/steady3d-tutorial/steady3d.cas) comprend les mots-clés pour la masse, bien qu'ils soient désactivés par le `/` au début de la ligne.
...
* Le paramètre de liste `PRESCRIBED ELEVATIONS : 0.;371.33` n'attribue aucune élévation à la limite **upstream** (numéro 1) et une élévation de 371,3 m a.s.l. à la limite **downstream** (numéro 2). Pour rappeler comment TELEMAC compte les limites ouvertes, lisez la boîte de commentaires dans {ref}`steady2d tutorial <tm2d-bounds>`.
...
* `5` — {cite:t}`spalart1992` one-equation {term}`RANS` model; **les deux** `HORIZONTAL TURBULENCE MODEL` **and** `VERTICAL TURBULENCE MODEL` doivent être fixés à `5`,
