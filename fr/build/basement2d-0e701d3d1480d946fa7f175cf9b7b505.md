...
* Un fichier d'entrée de décharge stable (hydrographe plat) pour l'état de la limite amont peut être téléchargé [here](https://github.com/hydro-informatics/materials-bm/raw/main/flows/steady-inflow.txt) (si nécessaire copier le contenu du fichier localement dans un éditeur de texte et enregistrer le fichier sous **steady-inflow.txt** dans le répertoire du projet).
...
* **Clic droit** sur **GEOMETRY** > **Ajouter le point REGIONDEF**
...
* Définissez les cinq régions par un ** clic droit** sur **index** > **Ajouter un élément**.
* Chaque élément **index [0]** obtient un nombre entier attribué correspondant au champ MATID dans le fichier forme des points de région (voir la section {ref}`regions` dans le tutoriel {ref}`qgis-prepro-bm`).
...
* **Clic droit** sur **GEOMETRY** > ** Ajouter le point STRINGDEF**.
...
* Définir l'élément STRINGDEF **[0]** avec :
* **nom** = `inflow`
* ** Direction en amont** = `right`
* Définir l'élément STRINGDEF **[1]** avec :
* **nom** = `outflow`
* ** Direction en amont** = `right`
...
* **Cliquez-droit** sur le nouvel article **BONDAIRE** et sélectionnez **Ajouter l'article STANDARD**.
* **Cliquez-droit deux fois** sur le nouvel élément **STANDARD** et sélectionnez **Ajouter l'élément** chaque fois. Ainsi, il devrait y avoir deux éléments **[0]** et **[1]** pour définir les conditions d'entrée et de sortie, respectivement.
* ** Définissez l'élément [0]** dans les conditions suivantes :
* Pour **nom** entrer `inflow`.
* Pour **string name** sélectionnez *inflow* (défini ci-dessus).
* Pour **type**, sélectionnez `uniform_in`
* **Cliquez-droit** sur **[0]** et sélectionnez **Ajouter l'élément 'pente'**.
...
* Dans la nouvelle ligne *décharge fichier*, cliquez sur le symbole du dossier pour sélectionner le fichier [steady-inflow.txt](https://github.com/hydro-informatics/materials-bm/raw/main/flows/steady-inflow.txt).
* **Définir l'élément [1]** dans les conditions suivantes :
* Pour **nom** appuyez sur `outflow`.
* Pour **string name**, sélectionnez le `outflow` STRINGDEF.
...
* **Cliquez-droit** sur le nouvel article **régions** et sélectionnez **Ajouter un article** (**cinq fois** pour les cinq régions).
* Attribuer les valeurs **friction** et **region name** énumérées à {numref}`Tab. %s <region-kst>` aux points **cinq régions**.
...
* **Cliquez-droit** sur le groupe **SIMULATION** dans l'onglet **Définir Simulation Exécuter** et sélectionner **Ajouter l'élément 'OUTPUT'**.
* **Cliquez-droit** sur le nouvel élément **OUTPUT** pour définir cinq types de sortie :
* **[0]** = `water_depth`
* **[1]** = `water_surface`
* **[2]** = `bottom_elevation`
* **[3]** = `flow_velocity`
...
* Le cadre **Standard Hardware** permet de basculer entre l'utilisation du processeur unique et celle du processeur multifils. L'option par défaut est multi-threaded, qui est fortement recommandé avec les ordinateurs contemporains.
...
* **Cliquez-droit** sur le groupe **RESULTATS** et sélectionnez **Ajouter l'élément 'EXPORT'**.
...
* Importer le maillage de qualité interpolé en hauteur {ref}`prepro-tutorial_quality-mesh-interp.2dm <qualm-interp>` (**Layer** > **Ajouter le calque** > **Ajouter le calque Mesh...**).
...
* Dans la fenêtre des propriétés **Layer**, allez au ruban **Source**.
* Dans le cadre **Données disponibles** (voir {numref}`Fig. %s <qgis-assign-meshdata>`) cliquez sur le bouton **Assigner les données supplémentaires au bouton Mesh** <img src="../../img/qgis/sym-add-meshdata.png"> et choisissez `results.xdmf`.
...
* Dans le cadre **Statical Dataset**, sélectionnez un **Scalar Dataset Group** et utilisez le temps maximum (c.-à-d. `625 d 00:00:00` dans le cas du temps de simulation $t$=15000 avec un intervalle de sortie de 1000).
...
* Dans l'onglet **Paramètres** (symbole du marteau dans le coin supérieur gauche surligné à {numref}`Fig. %s <symbology4u>`) trouvez la listbox **Groupes**.
* Dans la listbox **Groupes**, trouvez le paramètre à visualiser (par exemple **flow velocity**) et activez le symbole de contours.
...
Visualisez le paramètre flow velocity avec les commandes Symbology. Les cases rouges mettent en évidence les onglets et les entrées pertinents.
```
...
* Dans le cadre **Groupes de données**, cliquez sur le bouton **...** > **Sélectionnez Groupes de données disponibles** et sélectionnez **un paramètre** (par exemple **flow velocity**). Ensuite, cliquez sur le bouton **Retour** <img src="../../img/qgis/sym-go-back.png">. Assurez-vous que la toile **Groupes de données** contient seulement **1** sélectionné **option**. Sinon, l'outil créera un multibande messy {ref}`Raster <raster>`.
* Dans l'intervalle **Dataset time**, cochez le bouton **Dataset time step** radio et sélectionnez la dernière étape de simulation (c.-à-d. `625 d 00:00:00`).
* Dans le champ **Extend [facultatif]**, cliquez sur le bouton **...** > **Calculer à partir du calque** > **prepro-tutorial quality-mesh-interp**.
* Pour la taille **Pixel** tap `2.0` meters (plus ce nombre est grand, le plus gros sera le raster de sortie).
* Pour **Système de coordonnées des sorties** sélectionnez `Project CRS: ESRI:31494 - Germany_Zone_4`.
* Définissez un calque raster** en cliquant sur le bouton **...** > **Enregistrez dans Fichier**. Allez dans le répertoire cible (par exemple, `C:/Basement/steady2-tutorial/`) et entrez un nom de raster, comme `u-end.tif` (`u` pour la vitesse de flux, `end` pour la dernière étape du temps, et `.tif` pour {term}`GeoTIFF`). Cliquez sur **Enregistrer**.
* Cliquez sur le bouton **Run** pour commencer à rastériser l'ensemble de données de maillage.
...
* Dans le **Pipeline Browser**, **droit-clic** sur `results.xdmf` > **Ajouter un filtre** > **Alphabetic** (c'est-à-dire une liste de tous les filtres disponibles) > **Cell Centers**.
* Dans le **CellCenters1** **Propriétés**, cochez la case **Cellules Vertex** et cliquez sur le bouton maintenant vert **Appliquer** (voir {numref}`Fig. %s <fig-pv-cell-centers>`).
...
* Entrez un nom de fichier** (par exemple, `flow_velocity.csv`)
* Dans le champ déroulant **Files de type**, sélectionnez **Comma ou Tab Délimited Files(`*.csv *.tsv *.txt`)**.
* Cliquez sur **OK**.
* La fenêtre **Configure Writer (CSV Writer)** s'ouvre :
* Cochez la case **Choisir le tableau pour écrire**.
* Sélectionnez **points vitesse** uniquement (ou plus/autres paramètres).
...
Maintenant, un fichier *flow velocity.*{term}`CSV` contient des coordonnées point (coordonnées x, y et z) et flow velocity dans les directions *x* (flow velocity:0) et *y* (flow velocity:1). La direction flow velocity:2 (*z*-direction) est toujours nulle dans cette simulation 2d. Le fichier *flow velocity.*{term}`CSV` peut également être utilisé avec QGIS (par exemple, dans QGIS allez à **Layer** > **Ajouter un calque** > **Ajouter un calque de texte délimité...** > sélectionner *flow velocity.csv*, attribuer les colonnes et séparateurs corrects > cliquer **Ajouter**).

```{admonition} Challenge: Calculate the absolute velocity
Importer *flow velocity.csv* dans QGIS et calculer la vitesse absolue de flux $U$ à partir de $u_x$ (flow velocity:0) et $u_y$ (flow velocity:1) comme $U = \sqrt{u^2_x + u^2_y}$. Comment les champs de flux sont-ils conformes au raster `u-end.tif` (voir {ref}`bm-rasterize-output` section)?
```
...
* Activer en option l'environnement correspondant *Python* (conda ou vend).
* `cd` (changer le répertoire) dans le dossier de simulation.
* Exécuter `python BMv3NodestringResults.py`.
...
* ** Augmenter constamment la décharge dans une simulation régulière**<br> La définition de {ref}`upstream_direction <bm-geo-fin>` (par exemple, définie à tort comme `"left"` ou `"right"`) peut causer cette erreur.
* ** Débit inférieur à l'écoulement**<br>Augmenter le temps de simulation (voir la section {ref}`bm-sim-file`).
