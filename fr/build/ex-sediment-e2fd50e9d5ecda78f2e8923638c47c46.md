---
description: Exercice Python orienté objet pour le transport de sédiments en charge de fond en 1D en utilisant la formule Meyer-Peter et Müller, avec pandas DataFrames, cahiers Excel et hydraulique Manning-Strickler.
---
...
Cet exercice utilise des données hydrauliques moyennes de section unidimensionnelle (1d) produites avec le logiciel HEC-RAS du US Army Corps of Engineers {cite:p}`us_army_corps_of_engineeers_hydrologic_2016`, qui résout numériquement la formule Manning-Strickler pour toute forme de section transversale de flux. Dans cet exercice, *HEC-RAS* fournit les données hydrauliques nécessaires pour déterminer la capacité {term}`Transport solide <Sediment transport>` d'une section de canal, bien qu'aucune explication pour créer, exécuter et exporter des données à partir des modèles *HEC-RAS* ne soit donnée.

### Transport des sédiments

Fluvial {term}`Transport solide <Sediment transport>` peut être distingué en deux modes : (1) {term}`charge en suspension <Suspended load>` et (2) {term}`charge de fond <Bedload>` (voir {numref}`Fig. %s <transport-modes>`). Les particules plus fines dont le poids peut être transporté par le fluide (eau) sont transportées comme {term}`charge en suspension <Suspended load>`. Les particules grossières qui roulent, glissent et sautent sur le lit du canal sont transportées comme {term}`charge de fond <Bedload>`. Il y a un autre type de transport, la soi-disant charge de lavage, qui est plus fine que le gros {term}`charge de fond <Bedload>`, mais trop lourd (large) pour être transporté en suspension {cite:p}`einstein_bed-load_1950`.
...
La formule *Meyer-Peter & Müller* ne s'applique (comme toute autre formule {term}`Transport solide <Sediment transport>`) qu'à certaines rivières présentant les caractéristiques suivantes (étendue de validité) :
* 0.4 $\cdot$ 10$^{-3}$ m $< D_{char}$ < 28.6 $\cdot$ 10$^{-3}$ m
* 10$^{-4}$ m $< Fr <$ 639 ($Fr$ denotes the dimensionless [{term}`Nombre de Froude <Froude number>`](https://en.wikipedia.org/wiki/Froude_number))
* 0,0004 $< S_{e} <$ 0,02
* 0,0002 m$^3$/(s $\cdot$ m) $< q <$ 2.0 m$^3$/(s $\cdot$ m) ($q$ est la décharge de l'unité, c'est-à-dire $q=Q/[0. 5\cdot (b + B)]$)
* 0.25 $< s <$ 3.2

L'expression sans dimension pour {term}`charge de fond <Bedload>` $\Phi_b$ a été utilisée pour permettre le transfert d'informations entre différents canaux à travers les échelles en préservant la similitude géométrique, cinématique et dynamique. L'ensemble des paramètres sans dimension utilise les résultats de [Buckingham's $\Pi$ theorem](https://pint.readthedocs.io/en/stable/pitheorem.html) {cite:p}`buckingham_model_1915`.
Par conséquent, pour ajouter des dimensions à $\Phi_b$, il faut les multiplier avec le même ensemble de paramètres utilisés pour dériver l'expression sans dimension de *Meyer-Peter & Müller*. Leur ensemble de paramètres implique la taille caractéristique du grain $D_{char}$, la densité du grain $\rho_{s}$, et l'accélération gravitationnelle $g$. Ainsi, l ' unité dimensionnelle {term}`charge de fond <Bedload>` est (en kg/s et largeur du compteur, c ' est-à-dire kg/(s$\cdot$m):
...
The cross-section averaged {term}`charge de fond <Bedload>` $Q_{b}$ (kg/s) is then:
...
* `bedload.py` contiendra la classe `BedCore` avec les éléments de base que la plupart des formules {term}`charge de fond <Bedload>` ont en commun.
* `mpm.py` contiendra la classe `MPM`, qui hérite de `BedCore` et calcule {term}`charge de fond <Bedload>` comme décrit ci-dessus (Meyer-Peter & Müller 1948).
...
* Col. 09* I* Froude\# Chl* {term}`Nombre de Froude <Froude number>` du canal (si 1, erreur de calcul - ne pas utiliser!)
...
Ajouter une méthode pour convertir la valeur sans dimension {term}`charge de fond <Bedload>` transport $\Phi_b$ en valeur dimensionnelle (kg/s). En plus des variables définies dans la méthode `__init__`, la méthode `add_dimensions` exigera la largeur effective du canal $b_{eff}$ ({ref}`recall the above calculus <qb>`):
...
De nombreuses formules de transport {term}`charge de fond <Bedload>` impliquent la contrainte de cisaillement du lit sans dimension [$\tau_{x}$ (voir ci-dessus {ref}`definitions <taux>`) associée à un ensemble de paramètres hydrauliques moyens de section transversale. Par conséquent, appliquer la méthode de calcul `compute_tau_x` à `BedCore`:
...
### Écrire une classe d'évaluation Meyer-Peter & Müller charge de fond

Créez un nouveau script (par exemple `mpm.py`) et implémentez une classe `MPM` (**M**eyer-**P**eter & **M**üller) qui hérite de la classe `BedCore`. La méthode `__init__` de `MPM` devrait initialiser `BedCore` et écraser (rappeler {ref}`polymorphism`) les paramètres pertinents au calcul de {term}`charge de fond <Bedload>` selon Meyer-Peter & Müller (1948). En outre, l'initialisation d'un objet `MPM` devrait aller avec une vérification de la validité et le calcul du sans dimension {term}`charge de fond <Bedload>` transport $\Phi_b$ (voir ci-dessus {ref}`explanations of MPM <mpm>`):
...
Ajouter la méthode `check_validity` pour vérifier si les caractéristiques transversales fournies tombent dans la plage de validité de la formule Meyer-Peter & Müller (c.-à-d. pente, granulométrie, rapport de débit et profondeur de l'eau, et {term}`Nombre de Froude <Froude number>`):
...
```{note}
La méthode ici montrée `check_validity` prend le {term}`Nombre de Froude <Froude number>` comme argument d'entrée. Alternativement, assignez le {term}`Nombre de Froude <Froude number>` déjà dans `__init__` et utilisez `self.Fr`.
```

Pour calculer la méthode sans dimension {term}`charge de fond <Bedload>` transport $\Phi_b$ selon Meyer-Peter & Müller, implémenter une méthode `compute_phi` qui utilise la méthode `compute_tau_x` de `BedCore`:
...
3. Choisir et extraire 3 profils de `mpm_results` et tracer la dimensional {term}`charge de fond <Bedload>` transport $Q_{b}$ (axe-y) contre la décharge $q$ (axe-x).
```
