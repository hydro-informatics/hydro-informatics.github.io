---
description: Python exercise for scripting a habitat suitability map using GDAL raster operations, object-oriented programming, and geospatial analysis for ecohydraulics applications.
---
...
:class: tip
A couple of `gdal`-based functions for processing rasters and shapefiles were introduced in the lecture. This exercise re-uses some of these functions, which are available in the geo-processing code repository specifically for this ebook. The repository contains a functional block from [flusstools](https://flusstools.readthedocs.io).geotools (in the `geo_utils` folder) that enables to better understand and modify the behavior of [flusstools](https://flusstools.readthedocs.io).
