---
description: Tutorial for modeling unsteady (quasi-steady) discharge and flood hydrographs with Telemac2d, discretizing time-varying flows for 2D river hydraulic simulations.
---
...
**The following descriptions refer to section 4.1.3 in the [Telemac2d manual](https://gitlab.pam-retd.fr/otm/telemac-mascaret/-/blob/v9.0.0/documentation/telemac2d/user/telemac2d_user_9.0.pdf).**
...
The predictor-corrector schemes (**SCHEME FOR ...** keywords defined with `3`, `4`, `5`, or `15` rely on a parameter defining the number of iterations at every timestep for convergence (see the {ref}`steady2d tutorial <telemac2d-steady>`). For quasi-steady simulations, Telemac developers recommend setting this parameter to `2` or slightly larger (section 7.2.1 in the [Telemac2d manual](https://gitlab.pam-retd.fr/otm/telemac-mascaret/-/blob/v9.0.0/documentation/telemac2d/user/telemac2d_user_9.0.pdf)). Therefore, **add the following line to the steering file**:
...
Thus, re-running the simulation will write the fluxes across the two define control sections to a file called *r-control-flows.txt*. The [Telemac2d manual](https://gitlab.pam-retd.fr/otm/telemac-mascaret/-/blob/v9.0.0/documentation/telemac2d/user/telemac2d_user_9.0.pdf) provides explanations in section 5.2.2.
