---
description: Basic setup of the TELEMAC-GAIA steering file for morphodynamic simulations, covering mandatory parameters, boundary conditions, sediment properties, and riverbed characteristics.
---

(gaia-basics)=
# Basic Setup of Gaia

The following instructions refer to the setup of the above-created Gaia steering file (*gaia-morphodynamics.cas*), which needs some mandatory parameters and enables many more optional keywords settings. An overview of available keywords can be found in the [Gaia reference manual](https://gitlab.pam-retd.fr/otm/telemac-mascaret/-/raw/v9.0.0/documentation/gaia/reference/gaia_reference_9.0.pdf) and the Gaia dictionary file `/telemac/v9.0.0/sources/gaia/gaia.dico`. Similar to the Telemac2d or Telemac3d hydrodynamics steering file, the Gaia steering file can be distinguished between keyword groups for general (file-related), physical (sediment transport), and numerical parameters. This section introduces general parameters embracing the setup of boundary condition files and basic definitions of sediment and riverbed characteristics. The implementation of {term}`Bedload` and/or {term}`Suspended load` is covered in separate sections.
...
* Many more are listed in [Gaia reference manual](https://gitlab.pam-retd.fr/otm/telemac-mascaret/-/raw/v9.0.0/documentation/gaia/reference/gaia_reference_9.0.pdf).
```
...
Gaia derives mixed cohesive and non-cohesive sediment beds from the composition of the active layer. For mixed sediment, Gaia computes bedload transport only when the mass fraction of cohesive sediment in the active layer is less than 30%. Above this threshold, non-cohesive sediment can still be transported in suspension. The [Gaia manual](https://gitlab.pam-retd.fr/otm/telemac-mascaret/-/raw/v9.0.0/documentation/gaia/user/gaia_user_9.0.pdf) provides more information on the transport of mixed (cohesive and non-cohesive) sediment in section 3.2.1. In addition, riverbed consolidation can be simulated by defining the **BED MODEL** keyword with `2` (cf. [Gaia manual](https://gitlab.pam-retd.fr/otm/telemac-mascaret/-/raw/v9.0.0/documentation/gaia/user/gaia_user_9.0.pdf), section 3.3).
