...
* The **HIDING FACTOR FORMULA** keyword (for multi-class sediment) controls how finer particles are hidden by coarser ones:
  * `0`: constant hiding factor (**default**), which requires the per-class values to be given with the **CLASSES HIDING FACTOR** keyword
  * `1`: Egiazaroff formula
...
4. **Last**: Adjust **COEFFICIENT FOR DIFFUSION OF SUSPENDED SEDIMENTS** if plume spreading appears incorrect
