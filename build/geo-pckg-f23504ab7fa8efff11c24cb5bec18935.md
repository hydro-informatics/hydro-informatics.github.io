---
description: Overview of open-source Python geospatial libraries including GDAL, OGR, OSR, geopandas, rasterio, and flusstools for GIS data processing and spatial analysis.
---

# Geospatial Open Source Python Libraries

This section lists open-source packages for geospatial file manipulation with Python. The necessary packages are already installed [flusstools](https://flusstools.readthedocs.io). In addition, the following paragraphs provide explanations of relevant and optional packages for this eBook and how those can be installed.
...
* **Linux** users may follow the instructions for installing `gdal` and [flusstools](https://flusstools.readthedocs.io) with {ref}`pip and venv <pip-env>`.
* **Windows** users preferably install `gdal` and [flusstools](https://flusstools.readthedocs.io) in a {ref}`conda environment <conda-env>` through Anaconda.
...
(geojson-pckg)=
## geojson
The [geojson](https://pypi.org/project/geojson/) library is the most direct option for handling {ref}`geojson` data and is also already installed along with [flusstools](https://flusstools.readthedocs.io).
...
## pyshp
[pyshp](https://pypi.org/project/pyshp/) is another {ref}`shapefile <shp>` handling package, which builds on pure Python code (rather than wrappers) to simplify direct dealing with shapefiles in Python. `pyshp` is already installed along with [flusstools](https://flusstools.readthedocs.io).
...
Besides the above-mentioned packages, there are other useful libraries for geospatial analyses with Python (**`Packages in bold red font`** are installed along with [flusstools](https://flusstools.readthedocs.io)):
