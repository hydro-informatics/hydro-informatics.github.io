...
{% include requirements.html content="To complete the tutorial, the following software is needed (all software can be run on *Windows* and *Linux* platforms): <br><br>**- [*BASEMENT* v3.0.2](https://basement.ethz.ch/)**<br>**- [*QGIS v3.16*](geo_software.html#qgis)**, and<br>**- [*ParaView*](https://www.paraview.org/)**." %}
...
Prepare a `2dm` mesh file as described in the [QGIS data pre-processing](qgis-prepro.html) section.
...
{% include tip.html content="The defined project folder directory must not contain any dots nor special characters nor spaces. Only use letters, numbers, *_* (underscore) or *-* (minus) in folder names." %} 
...
- Define `STRINGDEF` item [1] as:
    * `name` = `Outflow`
    * `upstream_direction` = `right` 
    {% include note.html content="If you used [BASEmesh’s *Stringdef* tool](qgis-prepro.html#stringdef), the upstream direction must be defined as `right`." %}
...
{% include tip.html content="Ensure that the [*Crayfish* plugin](geo_software#plugins) is correctly installed and available in the *Toolbox*." %}
...
- Open `results.xdmf` in a text editor (e.g., [*Notepad++*](hy_others.html#npp))
...
- If not yet done, load the mesh file (here: [`finalmesh.2dm`](qgis-prepro.html#2dm)) by clicking on *QGIS*' `Layer` menu > `Data Source Manager` > `Mesh` tab and select `finalmesh.2dm`.
...
{% include image.html file="qgis-crayfish-installed.png" alt="rasterize crayfish qgis" caption="Open the Rasterize tool of the Crayfish plugin." %}
...
{% include image.html file="qgis-crayfish-exp.png" alt="setup rasterize crayfish plugin" caption="Settings to be made in Crayfish's Rasterize tool." %} 
...
{% include tip.html content="Facilitate the conversion of geospatial data with efficient *Python* algorithms (see the [geospatial *Python*](geo-python.html) section). Many *Python* conversion routines are also efficiently accessible and tailored for river analysis in the [*flusstools*](https://flusstools.readthedocs.io/) package." %}
...
After post-processing, the model still needs to be [calibrated and validated](calibration.html) before it can be used for scientific or engineering purposes in river ecosystem analyses. 