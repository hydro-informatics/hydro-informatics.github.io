...
* `B` für die untere Erhebung in (m a.s.l.)
* `E` für die untere Entwicklung in (m)
* `F` für {term}`Froude number`(-)
*`M` für die Größe (Länge) des bi-direktionalen (d.h. $x$ und $y$Wege) Einheit Sedimenttransport $\boldsymbol{q_s}$ (mehr in der Definition der {term}`Exner equation`) in (kg$\cdot$m$^{-1}\cdot$s$^{-1}$)
...
*`N` for unit bedload transport in $x$-direction$\boldsymbol{q_b}\cdot\cos\alpha$ in (kg$\cdot$m$^{-1}\cdot$s$^{-1}$) wo $\alpha$ den Winkel zwischen dem Längskanal ($x$) und dem festen Transportvektor $\boldsymbol{q_b}$ ist.
*`P` für Einzelbetttransporte in $y$-Richtung$\boldsymbol{q_b}\cdot\sin\alpha$ in (kg$\cdot$m$^{-1}\cdot$s$^{-1}$)
*`QSBL` für die Größe (Länge) des bi-direktionalen (d.h.$x$ und$y$-Anweisungen) Unit **bedload (nur)** transport$\boldsymbol{q_b}$ in (kg$\cdot$m$^{-1}\cdot$s$^{-1}$)
* `R` für den nicht-erodibaren Boden (m a.s.l.)
* `S` für Wasseroberflächenerhebung in (m a.s.l.)
* `TOB` für Bettscherbeanspruchung (N$\cdot$m$^{-2}$)
...
* `1` definiert Störwellen oder Thompson-Grenzungen
* `2` definiert Wandgrenzen
* `4` definiert freie (Neumann) Grenzen
* `5` definiert einen auferlegten Wert (Dirichlet) Grenze
...
* Offene Zuflussgrenzen (früher `LIHBOR=5`): `LIEBOR=5` und eingestellter Gleichgewichtszufluss in der Lenkdatei (siehe unten)
...
* `NCO` definiert **n*on-**co***hesive Sedimente
* `CO` definiert **co** Hesive Sediment
...
* **LACHER TEILNEHMER** definiert Erosionsflüsse in (kg$\cdot$m$^{-2}\cdot$s$^{-1}$) für jede Schicht (bis zu 20 Schichten).
...
* Die **aktive Schicht beschreibt das sofort bewegliche Flussbett**, wo **real-time**, **dynamische** Partikelverschiebung auftritt. Seine Dicke ist ein Vielfaches des charakteristischen Korndurchmessers.
* Die **-Disturban-Schicht umfasst Sandwellenverlauf** in Form von **scour und füllen** auf einer **-Eventskala**. Seine Dicke ist das 0,5-fache der Düne (oder reißig) Höhe.
...
## Geschiebtransport vs. hängende Last
