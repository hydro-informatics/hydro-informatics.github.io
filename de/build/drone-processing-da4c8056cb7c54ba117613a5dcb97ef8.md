...
* Arbeiten Sie vorzugsweise mit Orthobild; nur 3d-Modus, wenn starke Winde bläst
* Konten Sie etwa 20-30 Minuten Flugzeit pro Batterie (ca. 2 Stunden Nachladung), was bedeutet, dass 8 Batterien eine reibungslose Mapping ermöglichen
* Bodenkontrollpunkte können entfallen, wenn RTK-Daten verfügbar sind, aber wir empfehlen die Nutzung von Bodenkontrollpunkten
...
* Die **Accuracy*** auf **high*** für ein akzeptables Gleichgewicht von Auflösung und Rechenzeit einstellen. Beachten Sie, dass jede zusätzliche Erhöhung der Genauigkeit in einer Größenordnung längere Rechenzeiten führt.
* In **Referenzvorwahl** Wählen Sie **Quelle** aus und stellen Sie sicher, dass Sie GPS-Daten verwenden.
* **Erweitert** Rahmen:
* Die **Key Point Limit** Option ermöglicht die Definition von **charakteristischen Punkten** (d.h. Punkte, die der Ausrichtungsalgorithmus auf mehreren Bildern eindeutig identifizieren kann): stellen Sie sicher, dass 10.000 bis 40.000 charakteristische Punkte identifiziert werden können.
* die **Tie-Punktgrenze** definiert eine Mindestanzahl für Bindepunkte (d.h. Punkte, die der Ausrichtungsalgorithmus auf zwei benachbarten Bildern identifiziert): stellen Sie sicher, dass mindestens 1.000 bis 5.000 Bindepunkte identifiziert werden können.
* Verwenden Sie den **-fähigen Ausschluss von stationären Punkten**-Option, wodurch sichergestellt wird, dass Pixel, die die gleichen (Farben) Eigenschaften auf allen Bildern (d.h. stationär) haben, von der Ausrichtungserkennung ausgeschlossen sind. So werden beispielsweise Schmutzflecken auf der Kameralinse als Richttastenpunkte ausgeschlossen.
* Aktivieren Sie **Adaptive Kamera-Fitting-Modell* für unkorrigierte Kameraverzerrung.
* Hinterlassen Sie ** alle anderen Standard*-Felder as-is und klicken Sie auf **OK**, um die Ausrichtung zu starten. Je nach Anzahl und Größe der Bilder sowie der Berechnungskapazität Ihres Computers dauert die Ausrichtung 5-30 Minuten.
...
* bei sehr dichter Vegetation kann die Tiefenfüllung ** deaktiviert** sein, obwohl dies zu ** sehr langen Rechenzeiten** führen kann.
* Klicken Sie **OK**, um die dichte Punktwolke zu erstellen (kann wieder 5-30 Minuten dauern)
...
* Projektion:
* Verwenden Sie einen **geographic**-Typ für die Projektion auf einen {term}`GeoTIFF`raster, der mit den meisten GIS-Software kompatibel ist, wie z.B. {ref}`QGIS <qgis-tutorial>`.
* Die Standard-Projektion von Metashape ist `EPSG:4326` (weitere Informationen finden Sie im Abschnitt unter {ref}`projections in this eBook <prj>`), was der Projektion entspricht, die häufig mit Luftbildschnittstellen verwendet wird (z.B. *GoogleEarth*).
* Quellenangaben:
* Die **tie points** Option führt zu der **niedrigsten Auflösung**.
...
* Erweiterter Rahmen: automatisch vorgeschlagene Werte, um Konsistenz zu gewährleisten.
