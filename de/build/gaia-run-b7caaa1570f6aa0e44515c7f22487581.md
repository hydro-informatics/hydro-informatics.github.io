...
* Eine Gaia-Lenkdatei, wie [gaia-morphodynamics.cas](https://github.com/hydro-informatics/telemac/raw/main/gaia2d-tutorial/gaia-morphodynamics.cas).
...
*`--nctile=N`: Anzahl der Sub-Domains für die Domänenzersetzung (Standard gleich `ncsize`)
...
*`--clean`: Entfernen Sie temporäre Dateien, bevor Sie beginnen
````
...
* Lösung: Verwenden Sie Schema `14` oder `15` für suspendierte Sedimente, reduzieren Sie Zeitschritt, überprüfen Sie die Grenzbedingungen für suspendierte Last
```
...
* [Herunterladen von r-control-sections.txt](https://github.com/hydro-informatics/telemac/raw/main/gaia2d-tutorial/r-control-sections.txt) (Kontrollabschnitt Flussläufe)
...
2. Bei Verwendung eines anderen Projekts ist die On-the-fly-Reprojektion aktiviert
...
**Morphodynamik:**
- [ ] Sedimenttransport erfolgt, wenn Scherspannung kritische Werte überschreitet
- [ ] Belastungsrichtung richtet sich nach Hauptströmungsrichtung (mit erwarteten Kurvenabweichungen)
- [ ] Bed Evolution Größe ist plausibel (keine 10-m Scour Löcher am Basisstrom)
- [ ] Sediment-Massenbilanz wird geschlossen (die Gaia-Massenbilanzausgabe überprüfen)
- [ ] Nur bewegliche Sedimentklassen bewegen sich (Koarenanteile stabil bei niedrigen Strömen)
...
**Geschiebtransport-Messungen**: Sampler- oder Falldaten (selten, aber wertvoll)
...
* Für Modelle auf Basis der Formel {ref}`Meyer-Peter and Müller <gaia-mpm>` (d.h. unter Verwendung eines {term}`Shields parameter` für incipient sediment motion) kann das **CLASSES SHIELDS PARAMETERS** Schlüsselwort geändert werden:
* Wird die Erosion überbewertet, erhöhen Sie **CLASSES SHIELDS PARAMETERS**.
* Wird die Erosion unterschätzt, reduzieren Sie **CLASSES SHIELDS PARAMETERS**.
* Typische Reichweite: 0,03-0.06 für gleichmäßige Sedimente, bis zu 0,07 für gepanzerte Betten.

* Der **MPM COEFFICIENT** kann angepasst werden (Standard: `8`):
* Original Meyer-Peter und Müller Wert: `8`
* Wong-Parker-Korrektur für Beete: `3.97` (mit `CLASSES SHIELDS PARAMETERS : 0.0495`)
* Reduzieren Sie, um die gesamten Bettlast-Transportraten zu verringern.

* Mit der Steilkorrektur aktiviert und mit den {cite:t}`koch1980`Korrekturformeln das **BETA** Keyword von Equation {eq}`eq-qb-corr` (Standard ist `BETA : 1.3`):
* Wird die Erosion in gekrümmten Kanalabschnitten überproportioniert, verringern Sie **BETA**.
* Wird die Erosion in gekrümmten Kanalabschnitten unterschätzt, erhöhen Sie **BETA**.
* Typische Reichweite: 1.0-2.0.

* Um das Abscheidungs- und Erosionsmuster in Kurven (Riverbends) einzustellen, aktivieren Sie das **SECONDARY CURRENTS* Schlüsselwort und ändern Sie den **SECONDARY CURRENTS ALPHA COEFFICIENT*** Wert (vgl. {ref}`Secondary Currents <gaia-secondary>`):
* Standard: `1.0` (glättliches Bett)
* Für raue Betten: `0.75`
* Erhebt die spiralförmige Strömungsintensität und damit die laterale Sedimentumverteilung.

* Das **HIDING FACTOR FORMULA*** Keyword (für mehrstufiges Sediment) steuert, wie feinere Partikel von gröberen versteckt werden:
* `0`: konstanter Versteckfaktor (**default**), der die per-Klasse-Werte mit dem **CLASSES HIDING FACTOR**-Keyword angegeben werden muss
* `1`: Egiazaroff-Formel
...
1. **Erste**: Gesamte Transportgröße mit **CLASSES SHIELDS PARAMETERS** oder **MPM COEFFICIENT*
2. **Second*: Raumverteilung mit **BETA** und **SECONDARY CURRENTs ALPHA COEFFICIENT* anpassen
3. **Dritt**: Feinabstimmung mit **RATIO BETWEEN SKIN FRICTION UND MEAN DIAMETER**
4. **Letzte**: Verbergungs-/Belichtungseffekte für mehrstufige Modelle mit **HIDING FACTOR FORMULA**
...
- Erhöhung der Verkehrswege und Verbesserung der Ablagerung
...
- Erhöhung der Abscheidung bei höheren Scherbelastungen
- Standard von `1000` N/m2 deaktiviert die Abscheideschwelle effektiv

* **LACHER TEILNEHMER** (Eisenkurs konstant $M$):
...
- Verringern Sie die Erosion (untere Schwelle)
...
- Untere Werte konzentrieren Sedimentschläuche

```{admonition} Recommended calibration sequence for suspended load
:class: tip

1. **Erste**: Passende suspendierte Sedimentkonzentrationen an Messstationen durch Einstellen ** LAYERS PARTHENIADES CONSTANT** (erosion) und **CLASSES SETTLING VELOCITIES** (deposition)
2. **Second*: Anpassen ** RECHTSSACHE CRITICAL EROSION SHEAR STRESS THE MUD**, um zu kontrollieren, wo Erosion initiiert
3. **Dritte**: Feinabstimmung **KLASSEN CRITICAL SHEAR STRESS FÜR MUD DEPOSITION** zur Abstimmung von Abscheidungsmustern
...
* **Spatial Split**: Kalibrieren Sie auf dem vorgeschalteten Teil, validieren Sie nachgeschaltet (oder umgekehrt)
