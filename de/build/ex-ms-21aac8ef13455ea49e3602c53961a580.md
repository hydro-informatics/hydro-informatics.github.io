---
description: Python-Übung für 1D-Open-Channel-Hydraulik mit der Manning-Strickler-Formel, Implementierung von Schleifen und Funktionen zur Berechnung von Bühnenentladungsbeziehungen.
---
...
```{admonition} Goals
Schreiben Sie ein grundlegendes Skript und verwenden Sie Loops. Schreiben Sie eine Funktion und analysieren Sie optionale Keyword-Argumente (`**kwargs`).
```

```{admonition} Requirements
:class: attention
* Python* Bibliotheken: *Math* (Standardbibliothek). Lesen und verstehen Sie, wie {ref}`loops <sec-pyloop>` und {ref}`chpt-functions` in Python funktionieren.
```
...
* $u$ die querschnittsgemittelte Strömungsgeschwindigkeit in (m/s)
...
* $S$ ist die hypothetische Energiesteigung (m/m), von der angenommen werden kann, dass sie der Kanalsteigung für stetige, gleichmäßige Strömungsverhältnisse entspricht.
...
Daher ergeben sich $A$ und $P$ aus den folgenden Formeln:
...
## Flexibilisierung
Machen Sie die Funktion durch die Implementierung von (optional) {ref}` keyword arguments <kwargs>` flexibler, so dass ein Benutzer wahlweise entweder $D_{90}$ (`D90`), den *Strickler*-Koeffizienten $k_{st}$ (`k_st`) oder *Mannings* $n_m$ (`n_m`) bereitstellen kann.
...
* Schreibe eine neue Funktion `def interpolate_h(Q, b, m, S, **kwargs):`
...
* Use a `while` loop until the error margin is negligible small (e.g., `while eps > 10**-3:`) and calculate the :
	...
	- aktuelle Entladungsschätzung (basierend auf `h`): `Qk = A ** (5/3) * sqrt(S) / (n_m * P ** (2 / 3))`
	- Fehlermeldung `eps = abs(Q - Qk) / Q`
...
* Return `h` und `eps` (oder berechnete Entladung `Qk`)
