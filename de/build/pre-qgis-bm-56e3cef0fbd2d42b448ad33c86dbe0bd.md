...
* Noch im **Plugins* Popup-Fenster gehen Sie zurück zum **All** Tab und geben Sie im Suchfeld `basemesh` ein.
...
* In *QGIS*' **Browser** Panel finden Sie das **Project Home** Verzeichnis, in dem Sie das Digitales Oberflächenmodell (DOM) *tif* heruntergeladen haben.
...
* {ref}`Line Shapefile <create-line-shp>` mit Modellgrenzen und internen Breaklines zwischen Modellregionen mit unterschiedlichen Eigenschaften (Abschnitt {ref}`boundary`)
* {ref}`Line Shapefile <create-line-shp>` mit Modellgrenzen für die Zu- und Abflussbedingungen (Abschnitt {ref}`liquid-boundary`) und a
...
* Starten Sie eine Linie auf der linken Bank (linke Seite der unteren Figur) und bewegte Ostern (d.h. nach rechts) um sieben weitere Punkte über den Fluss zu machen.
* Der **siebte Punkt** muss **Koincide*** mit dem Ende der rechten Bank ** Floodplain Grenzlinie**.
* So kommt der stromaufwärts liegende Fluss von der rechten Seite der Zuflussgrenzlinie (d.h. die stromaufwärts gerichtete Strömungsrichtung wird `right` für das numerische Modell sein).
...
* Als nächstes ziehen Sie eine Abflussgrenze (siehe auch {numref}`Fig. %s <outflow-boundary>`):
...
**Region Markerschicht** = **Regionen-Punkte*** (siehe {ref}`regions`).
...
* Im **Basic**-Tab finden Sie den **Höhequelle**-Rahmen und aktivieren Sie die **Aktivierung über Digitales Oberflächenmodell (DOM) (Raster)** Funktaste.
* Wählen Sie **dem.tif** GeoTIFF (siehe {ref}`get-dem` Sektion) als **Rasterschicht**.
* Im **Output**-Rahmen klicken Sie auf die **Browse**-Schaltfläche, um einen Ausgabenetznamen im `/Project Home/`-Verzeichnis zu definieren, z.B. **prepro-tutorial quality-mesh-interp.2dm***
* Klicken Sie **** um das höheninterpolierte Mesh zu erstellen.
...
1. Konvertieren Sie die Digitales Oberflächenmodell (DOM) in eine Punktformdatei (**raster in den Vektor**) und stellen Sie sicher, dass die Punktattributtabelle mit Erhebungen gefüllt ist.
2. Verwenden Sie das **TIN Mesh Creation* Werkzeug, um ein TIN-Höhengitter mit Höhendaten zu erzeugen.
3. Wählen Sie im BASEmesh's **Interpolation**-Tool die Option **Interpolation über Elevation Mesh** und wählen Sie das zuvor erstellte TIN Elevationsnetz aus.
