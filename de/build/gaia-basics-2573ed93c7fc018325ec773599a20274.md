...
Grafische Ausgabevariablen, die sich auf den Sedimenttransport beziehen, können mit dem Schlüsselwort **VARIABLES FOR GRAPHIC PRINTOUTS** für {term}`Geschiebe <Bedload>` und / oder {term}`Schwebstoff <Suspended load>` und den folgenden Listenoptionen definiert werden:

* `B` für die untere Höhe in (m a.s.l.)
* `E` für Bottom Evolution in (m)
* `F` für {term}`Froude-Zahl <Froude number>` (-)
* `M` für die Größe (Länge) der bidirektionalen (dh $x$ und $y$ Richtungen) Einheit Sedimenttransport $\boldsymbol{q_s}$ (lesen Sie mehr in der Definition von {term}`Exner-Gleichung <Exner equation>`) in (kg$\cdot$m$^{-1}\cdot$s$^{-1}$)
* `MU` für den Hautreibungskoeffizienten (als Funktion von {ref}`skin friction correction factors <c-friction>`, beschrieben im Abschnitt über Geschiebefracht)
* `N` für den Geschiebetransport in $x$-Richtung $\boldsymbol{q_b}\cdot\cos\alpha$ in (kg$\cdot$m$^{-1}\cdot$s$^{-1}$), wobei $\alpha$ der Winkel zwischen der Längsachse ($x$) und dem festen Transportvektor $\boldsymbol{q_b}$ ist.
* `P` für den Transport von Geschiebefracht in $y$-Richtung $\boldsymbol{q_b}\cdot\sin\alpha$ in (kg$\cdot$m$^{-1}\cdot$s$^{-1}$)
...
* (9) `Q2BOR`: **Gaia-spezifischer** Feststoffaustrag (verwendet für Geschiebefracht, in kg$\cdot$m$^{-1}\cdot$s$^{-1}$) oder Sedimentkonzentration in der Nähe des Bettes (in g$\cdot$L$^{-1}$) im Falle einer Schwebstofffracht (oder eines zusammenhängenden Sediments)
* (10) `EBOR`: **Gaia-spezifische** Flussbetthöhe (m a.s.l.)
* (11) `CBOR`: **Gaia-spezifische** Gleichgewicht suspendierte Sedimentkonzentration (in g$\cdot$L$^{-1}$) für Schwebstofffracht Modellierung (lesen Sie mehr in der {ref}`section on suspended load <gaia-sl>`)
* (12) Punktnummer
...
Um den Gleichgewichts-Feststoffaustrag (Geschiebefracht) und die suspendierte Sedimentkonzentration an den Zuflussgrenzen zu definieren, **fügen Sie der Gaia-Lenkdatei Folgendes hinzu **:
...
Bestimmte Sedimenttransportformeln zur Simulation von {term}`Geschiebe <Bedload>` oder {term}`Schwebstoff <Suspended load>` beziehen sich auf die betrachteten Phänomene und ihre Implementierung in die Gaia-Lenkungsdatei wird in den nächsten Abschnitten erläutert.
...
:class: note
In Gaia sind die Begriffe **aktive Schicht** und **Mischschicht** synonym: Beide beziehen sich auf die oberste Sedimentschicht, die in direktem Kontakt mit der Wassersäule steht. Diese Schicht liefert Material, das als Geschiebefracht oder Schwebstofffracht transportiert werden kann, und es erhält abgelagertes Sediment. Die Schichten unterhalb der aktiven Schicht werden **substratum** genannt.
```
...
* **Während der Erosion**: Sedimentmasse wird aus der aktiven Schicht für den Transport oder die Suspension von Geschiebefracht entfernt. Um die Zieldicke aufrechtzuerhalten, überträgt Gaia Masse vom Substrat (der ersten nicht leeren Schicht unterhalb der aktiven Schicht) in die aktive Schicht. Das übertragene Material hat die Zusammensetzung des Substrats, was die Zusammensetzung der aktiven Schicht im Laufe der Zeit verändern kann.
...
Sediment transport modeling quickly becomes computationally expensive. Therefore, it is important to be clear about the primary type of sediment transport mode and to activate only the most important phenomenon (i.e., either {term}`Geschiebe <Bedload>` or {term}`Schwebstoff <Suspended load>`). For this reason, answer the question ***What type of sediment transport phenomenon is predominant in the model?*** If you are not sure about the answer to this question, revise the section on {ref}`sediment transport modes <gaia-seditrans>`. In addition, here are some practice-oriented suggestions:
