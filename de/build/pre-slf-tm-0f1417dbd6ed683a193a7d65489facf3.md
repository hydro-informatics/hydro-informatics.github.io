...
* Folgen Sie den Installationsanweisungen für {ref}`qgis-install` in diesem eBook.
* Lesen (oder beobachten) und verstehen Sie das eBook {ref}`qgis-tutorial`.
...
* In *QGIS*' **Browser** Panel finden Sie das **ProjectHome** Verzeichnis, in dem Sie das Digitales Oberflächenmodell (DOM) *tif* heruntergeladen haben.
* Ziehen Sie den Digitales Oberflächenmodell (DOM) *tif* aus dem **ProjectHome** Ordner in das QGIS' **Layer* Panel.
* Um später die Delineierung bestimmter Regionen des Flussökosystems zu erleichtern, fügen Sie eine {ref}`satellite imagery basemap <basemap>` (XYZ-Fliese) unter der {term}`DEM` hinzu und passen Sie die Layer-Symbologie an.
...
* Im **File Format** Rahmen, stellen Sie sicher, **Custom-Abgrenzer*** auszuwählen und überprüfen Sie die **Space**-Abgrenzerbox.
* Im Rahmen **Record und Fields Options** setzen Sie die **Anzahl der Header-Linien auf Disard** an `13` und überprüfen Sie die **Erste Platte hat Feldnamen** Box.
* Im Rahmen **Geometrie Definition** wählen Sie `:EndHeader` als **X Feld*, `field_2` als **Y Feld* und `field_3` als **Z Feld**. Wählen Sie `Project CRS: ESRI:32633 - WGS 84 / UTM zone 33N` als **Geometry Koordinatenreferenzsystem***.
* Klicken Sie auf **** und ** schließen** das *Datenquellen-Manager*-Fenster.
...
* Stellen Sie sicher, dass die Leitungen perfekt mit den vorkonstruierten Überschwemmungsgrenzlinien übereinstimmen (Snapping ist erforderlich); so müssen die Trennlinien des Hauptkanals und die Überschwemmungsgrenzlinien die Überschwemmungen ohne Lücke zwischen den Leitungen umschließen.
* **Kieselbstleitungen*:
...
* Finden Sie die rauen Blockrampen (abstoßende Gewässer) in der Satellitenbildkarte und delineieren Sie sie durch Linien über den benetzten Hauptkanal.
* Stellen Sie sicher, dass die Linie mit den Hauptkanalbruchlinien perfekt zusammenfällt; so müssen die harten Bruchlinien des Hauptkanals und die Blockrampenbruchlinien die Blockrampen ohne Lücke zwischen den Leitungen umschließen.
...
* Eine Zuflussgrenze zeichnen (leichte blaue Linie links von {numref}`Fig. %s <tm-shapefiles>`):
* Zoom in den Zuflussbereich der Digitales Oberflächenmodell (DOM)-Grenze, wo zwischen** die oben erstellten **-Flottplain-Grenzlinien** liegen.
* Starten Sie eine Linie auf einer Bank (oben der untenstehenden Figur) und fahren Sie zur anderen Bank, um etwa sieben weitere Punkte über den Fluss zu machen.
* Der **letzte Punkt** muss **Koincide*** mit dem Ende der **Floodplain Grenzlinie** der anderen Bank.
* **Finalize** die Zeile mit **right-click*** und geben Sie `Inflow` im **type*-Feld und `inflow` im **stringdef**-Feld ein.
* Ziehen Sie eine Abflussgrenze (leichte blaue Linie rechts von {numref}`Fig. %s <tm-shapefiles>`):
* Zoom in den Abflussbereich der Digitales Oberflächenmodell (DOM)-Grenze, wo sich ein **-Gap zwischen** die oben erstellten **-Floodplain-Grenzlinien* befindet.
* Starten Sie eine Linie auf einer Bank (oben der untenstehenden Figur) und fahren Sie zur anderen Bank, um etwa sieben weitere Punkte über den Fluss zu machen.
* Der **letzte Punkt** muss **Koincide*** mit dem Ende der **Floodplain Grenzlinie** der anderen Bank.
* * **Finalize** die Zeile mit **right-click** und geben Sie `Outflow` im **type*-Feld und `outflow` im **stringdef**-Feld ein.
...
**Region Markerschicht** = **Regionen-Punkte*** (siehe {ref}`make-tm-shp`).
**Aktivieren Sie das MATID-Feld** und wählen Sie das *regions-points* formfile's **MATID-Feld** aus.
...
* Klicken Sie auf die **Run* Schaltfläche, um das Qualitätsmaschen zu erstellen.
...
* der **WorkSpace** Browser (auf der linken Seite des Fensters),
* den **Dateneintrag im **WorkSpace***, in dem die Dateiobjekte aufgeführt werden,
...
* Klicken Sie auf **Open**, um den Import abzuschließen.
...
* Im Popup-Fenster (*Properties of:new Selafin*) klicken Sie auf **OK** und ein **neues Selafin* Objekt wird im WorkSpace's **Data Items** angezeigt.
* **Rechtsklick** auf das **neue Selafin** Objekt und wählen ** Variable hinzufügen...**
* Nehmen Sie die folgende Aktion im **Add New SELAFIN Variable** Fenster:
* Im Feld **Mesh** wählen Sie das oben importierte (oder geöffnete) Netz (z.B. `prepro-tutorial_quality-mesh-utm33n.2dm`).
* Im Feld **Name** wählen Sie **BOTTOM***.
* Im Feld **Units** wählen Sie **M** (d.h. Meter).
* Halten Sie alle anderen Voreinstellungen und klicken Sie auf **OK**.
...
* Im geöffneten Popup-Fenster (**Verfügbare t3s Objekte**), wählen Sie das oben erstellte **BOTTOM*-Netz (d.h. das Netz mit Höheninformationen) und klicken Sie auf **OK***. Ein neues **BOTTOM BC* Objekt wird im **Data Items** Baum des WorkSpace auftreten.
...
* Hervorhebung des **-Grenzes (LIHBOR)** Eintrags des **-Grenzes** (oder BOTTOM BC)-Objekts im **Data Items**-Baum und
