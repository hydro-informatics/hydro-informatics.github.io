...
* Fuzzy-Regelsets {cite:p}`noack_ecohydraulics_2013`
```
...
* `start_logging()` beginnt mit der Anmeldung an ein Logfile (`logfile.log`) und an der *Python*-Konsole auf der `logging.DEBUG`-Ebene.
````
...
- Wenn `constant_or_raster` numerischer Wert ist, dann rufen Sie `contant_or_raster.array` an, dass eine `AttributeError` und die `__truediv__`-Methode in die `except AttributeError`-Anweisung fällt, wobei `self.array` einfach durch `constant_or_raster` geteilt wird.
* Die Methode gibt das Ergebnis der Pseudo-Privatmethode `self._make_raster("div")` ([Recall *PEP 8* {ref}`chpt-style` zurück, die einer neuen `Raster`Instanz der eigentlichen `Raster`Instanz entspricht, geteilt durch `constant_or_raster`. Die neue `Raster`Instanz ist eine temporäre *GeoTIFF*-Datei im *cache*-Ordner (Recall the {ref}`cache function <funs>`). So sieht die pseudo-private Methode `_make_raster(self, file_marker)` aus:</a>
...
Diese Funktion:
* Benutzt das *string*-Argument `file_marker`, um es dem Dateinamen der Basis *GeoTIFF* zusammen mit einem zufälligen, vier Zeichen langen *string* (Recall the {ref}`create_random_string <funs>`) hinzuzufügen. `file_marker` ist einzigartig für jeden implementierten Betreiber. Für die Methode `__truediv__` verwenden Sie `file_marker="div"`. So wird der temporäre *GeoTIFF*-Dateiname als `cache_folder + self.name + f_ending` (z.B. `"C:\Excercise-geco\__cache__\velocity__divhjev__.tif"`) definiert.
* Gilt für die {ref}`create-raster`-Funktion von `geo_utils`, um den temporären *GeoTIFF* an den `__cache__`-Ordner mit dem räumlichen Referenzsystem des ursprünglichen Rasters zu schreiben.
...
* Die `parameters`-Liste enthält `"velocity"` und `"depth"` (gemäß der `par_dict` im `config.py`Script),
* die Dateipfade werden korrekt definiert und
