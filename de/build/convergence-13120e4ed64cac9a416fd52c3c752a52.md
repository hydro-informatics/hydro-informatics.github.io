...
* Füllen Sie die {ref}`Telemac steady 2d tutorial <telemac2d-steady>` (oder ähnliches) aus.
...
* Die Telemac-Simulation muss mit der `s`fahne ausgeführt worden sein (weitere Details unten):

```fortran
telemac2d.py [STUDY-NAME].cas -s
```

* Eine Installation von Python zusammen mit der `numpy`, `pandas` und `matplotlib` Bibliotheken ({ref}`see the Python installation guide <install-python>`), obwohl`flusstools` nicht erforderlich ist.
...
* langsam *sublinear* Konvergenz bei $\iota$ = 1 ** und**$c_{\varepsilon}$ = 1
