...
* Debian Linux 10 (Buster) installiert auf einer virtuellen Maschine (weiterlesen unter {ref}`software chapter <chpt-vm-linux>`).
```
...
* `VARIABLES FOR 3D GRAPHIC PRINTOUTS`: `U,V,H,S,Q,F` - viele weitere Optionen finden Sie in Abschnitt 3.12 der [Telemac3d manual](https://gitlab.pam-retd.fr/otm/telemac-mascaret/-/raw/v9.0.0/documentation/telemac3d/user/telemac3d_user_9.0.pdf)
* `VARIABLES FOR 2D GRAPHIC PRINTOUTS`: `U,V,H` - viele weitere Optionen finden Sie in Abschnitt 3.13 der [Telemac3d manual](https://gitlab.pam-retd.fr/otm/telemac-mascaret/-/raw/v9.0.0/documentation/telemac3d/user/telemac3d_user_9.0.pdf)
...
* **Zeit**:
  + `TIME STEP`: `1.0` definiert den Zeitschritt als ein Vielfaches von grafischen/listigen Ausdrucksperioden.<br>*Verwenden Sie kleine genug und ausreichende Zeitschritte, um die Rechenstabilität zu erreichen/zu erhöhen und die Recheneffizienz zu steigern.*
  + `NUMBER OF TIME STEPS`: `8000` definiert die Gesamtsimulationslänge. <br>* Begrenzen Sie die Anzahl der Zeitschritte auf ein Minimum (z.B. bis die Gleichgewichtsbedingungen in einer stetigen Simulation erreicht sind).*
...
* `LAW OF BOTTOM FRICTION`: `5`
* `LAW OF FRICTION ON LATERAL BOUNDARIES`: `5`, die gut an Modell-Naturbanken angewendet werden kann, oder an `0` (no-slip) für Symmetrie.<br>* Beachten Sie, dass die Randbedingungen-Datei die `LIUBOR` und `LIVBOR` für die `leftwall` und `rightwall` Randkanten auf Null setzt, um Reibung zu ermöglichen.
* `FRICTION COEFFICIENT FOR THE BOTTOM`: `0.1` entspricht dem 3-fachen eines hypothetischen *d90* (Körnerdurchmesser davon 90% des Oberflächenkorngemisches feiner) nach {cite:p}`vanrijn2019`.
...
* `5` — {cite:t}`spalart1992` one-equity {term}`RANS`model; **both**`HORIZONTAL TURBULENCE MODEL` ** und*`VERTICAL TURBULENCE MODEL` muss auf `5`
