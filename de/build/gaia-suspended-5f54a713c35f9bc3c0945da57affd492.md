...
Um die Simulation der hängenden Last zu aktivieren, fügen Sie der Gaia-Lenkdatei Folgendes hinzu:
...
Für suspendierte Ladung ist die Definition zusätzlicher Sedimenteigenschaften für jede Sedimentklasse erforderlich (oder aktiviert).
...
* `2` für die {cite:t}`bijker1992` Formel:
  - Berechnet die Schwebelastkonzentration als Funktion der Bettlast und einer Bezugshöhe der Hautreibung
  - Erfordert, dass {ref}`bedload calculation <gaia-bl>` aktiviert ist (`BED LOAD FOR ALL SANDS : YES`)
  - Geeignet für kombinierte Bettlast-Suspended Load Berechnungen
...
* `4` für die Formel {cite:t}`soulsby1997`-@-{cite:t}`rijn2007`:
  ...
  - Lesen Sie mehr über aufgehängte Last und Wellen in Abschnitt 5.1 des [Gaia Manual](https://gitlab.pam-retd.fr/otm/telemac-mascaret/-/raw/v9.0.0/documentation/gaia/user/gaia_user_9.0.pdf)]
...
Die meisten numerischen Parameter für die Modellierung der Schwebelast hängen von den hydrodynamischen Telemac2d/3d-Lenkdateidefinitionen ab. Zusätzliche Schlüsselwörter, die sich direkt auf die Simulation der hängenden Last auswirken, sollten in der Gaia-Lenkdatei deklariert werden.
...
* `2` für das nicht-konservative *Streamline Upwind Petrov Galerkin* (SUPG) Schema, das die {term}`CFL` Bedingung verwendet und weniger diffusiv ist als das *Characteristics* (`1`) Schema.
...
* 3D-Modell des zusammenhängenden Schwebetransports mit vertikalem Rouse-Profil (vgl. [Gaia manual](https://gitlab.pam-retd.fr/otm/telemac-mascaret/-/raw/v9.0.0/documentation/gaia/user/gaia_user_9.0.pdf), Abschnitt 2.1.2): **rouse-t3d/**
