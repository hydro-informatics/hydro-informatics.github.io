...
This section defines internal numerical parameters for the {term}`Advektion <Advection>` and  {term}`Diffusion` solvers.
...
Similar to {term}`Advektion <Advection>`, the above keywords can be used to define {term}`Diffusion` steps (replace `ADVECTION` with `DIFFUSION` in the keywords), where a value of `0` can be used to override the default value of `1` and disable diffusion.
...
* `1` ermöglicht das Upwinding mit dem klassischen SUPG-Schema (Standard; empfohlen, wenn die Bedingung {term}`CFL-Zahl <CFL>` unbekannt ist), und
