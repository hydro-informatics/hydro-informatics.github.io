...
* *castellatedMesh* ermöglicht die Schaffung eines kastellierten (d.h. verfeinerten) Netzes.
* * *snap* ermöglicht die Oberflächenschnappstufe.
* * AddLayers* ermöglicht das Einführen der Oberflächenschicht.
* * Geometrie* ist ein Unterdiktionär aller verwendeten Oberflächengeometrien.
...
* * nSolverIter*: definiert die Anzahl der Mesh-Verschiebe Iterationen.
* **nRelaxIter*: definiert die Anzahl der Relax-Iterationen während des Snappings.
...
* <span style="color: #f2003c ">***maxNonOrtho***</span>: maximaler Face-Non-Torthogonalitätswinkel, berechnet als normalisiertes Punktprodukt des Oberflächenvektors einer Zelle $i$ und des controid-to-centroid Vektors zweier benachbarter Zellen $i$ und $j$.
* <span style="color: #e68a19 ">***maxBoundarySkewness***</span>: Maximale Grenzskewness.
...
*<span style="color: #f2003c ">***minFaceWeight***</span>: Mindestgewicht der Gesichtsinterpolation, berechnet als Minimum der projizierten Entfernungen $d_{prj}$ zwischen zwei benachbarten Zellen $i$ und $j$, insbesondere $\min (d_{prj, i}, d_{prj, j}) / (d_{prj, i} + d_{prj, j})$
*<span style="color: #f2003c ">***minVolRatio***</span>: Verhältnis des Mindest- und Höchstvolumens $V$ der Nachbarzellen$i$ und $j$, d.h. $\min(V_i, V_j) / \max(V_i, V_j)$.
...
* Führen Sie den `blockMesh` Befehl aus, um das Hintergrundnetz zu erstellen:

```
user@user123:~/OpenFOAM-9/channel/Mesh$ blockMesh
```

* Führen Sie den `surfaceFeatures` Befehl aus, um die *.obj* und *.eMesh*-Dateien aus den ursprünglich importierten {term}`STL`-Dateien zu erstellen. Diese Dateien werden im Ordner *extendedFeatureEdgeMesh* (channel/constant/extendedFeatureEdgeMesh) gespeichert.
```
user@user123:~/OpenFOAM-9/channel/Mesh$ surfaceFeatures
```

* Für Parallelläufe verwenden Sie den Befehl `decomposePar`, um die Geometrie in einzelne Geometrien für jeden MPI-Prozess zu zerlegen.

```
user@user123:~/OpenFOAM-9/channel/Mesh$ decomposePar
```

* Führen Sie den `snappyHexMesh` Befehl aus, um das Netz zu erzeugen:
...
* Für Parallelläufe verwenden Sie den Befehl `reconstructParMesh`, um die Geometrie zu rekonstruieren.
