...
* Quasi-Stoff (near-census unsteady) Strömungsbedingungen (z.B. wichtig für die Modellierung eines Flut-Hydrographen) in der {ref}`unsteady Telemac2d tutorial <chpt-unsteady>`. Dieses Tutorial baut auf dem stationären Telemac2d Tutorial.
* Ein rein hydrodynamisches 3d-Modell in der {ref}`Telemac3d tutorial <chpt-telemac3d-slf>` einrichten.
* Kopplerhydrodynamik (d.h. Telemac2d oder Telemac3d) mit Morphodynamik (d.h. {term}`Sediment transport`) in der {ref}`Gaia tutorial <tm-gaia>`.
...
*{ref}`qgis-install` und das BASEmesh-Plugin, die im {ref}`QGIS pre-processing tutorial <slf-prepro-tm>` (** der bevorzugten Wahl des Autors*) dargestellt sind.
* Die Software des National Research Council Canada's {ref}`Blue Kenue <bluekenue>` GUI (vor allem für *Windows*).
...
* Steuerungsdatei
+ Dateiformat: `*.cas`
+ Bereiten Sie entweder mit {ref}`Fudaa PrePro <fudaa>` vor oder verwenden Sie einen Texteditor (z.B. {ref}`npp`).
* Geometriedatei
...
+ Vorbereiten `*.slf` Geometrien mit {ref}`QGIS <qgis-tutorial>`or{ref}`Blue Kenue <bluekenue>` (weiterlesen unter {ref}`TELEMAC pre-processing tutorial <bk-create-slf>`).
+ Bereiten Sie `*.med` Geometrien mit {ref}`SALOME <salome-install>`.
* Bounding Bedingungen
...
+ Bereiten Sie `*.bnd`/`*.bcd` Dateien entweder mit {ref}`SALOME <salome-install>` oder mit einem Texteditor vor.
...
* Phase-Decharge-Beziehungsdatei
+ Dateiformat: `*.tbl` oder `*.txt` (`ASCII`)
