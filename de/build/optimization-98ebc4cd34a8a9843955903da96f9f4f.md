...
* Verwenden Sie die {ref}`ACCURACY and MAXIMUM ITERATION <tm2d-accuracy>` Keywords, um schnellere Konvergenz zu erzielen.
* Deaktivieren `TIDAL FLATS`, obwohl die Deaktivierung {ref}`tidal flats <tm2d-tidal>` nicht empfohlen werden kann, um physikalisch sinnvolle und stabile Modelle zu liefern.
* Bei Verwendung des GMRES-Lösers (`SOLVER : 7`) kann die Variation der {ref}`solver options <tm2d-solver-pars>` dazu beitragen, die Gesamtberechnungszeit zu reduzieren.
* Stellen Sie sicher, dass Sie das Standard `MATRIX STORAGE : 3`keyword verwenden.
* Verwenden Sie eine frühere Simulation (z.B. mit einem gröberen Mesh), um das Modell mit den `COMPUTATION CONTINUED : YES` und `PREVIOUS COMPUTATION FILE : *.slf`Schlüsselwörtern einzuleiten (siehe Abschnitt 4.1.3 in der [Telemac2d manual](https://gitlab.pam-retd.fr/otm/telemac-mascaret/-/blob/v9.0.0/documentation/telemac2d/user/telemac2d_user_9.0.pdf)).
...
*`FREE SURFACE GRADIENT` - default is `1.0`, aber es kann auf `0.1` reduziert werden, um Stabilität zu erreichen (jeweils beginnen Sie mit inkremental ab, wie ein Wert von `0.9`).
...
* Kaltstarts (d.h. {ref}`defining initial conditions with the INITIAL CONDITIONS keyword in the steering file <tm2d-init-dry>`) dürfen nicht konvergieren. Daher auch
- die `PRESCRIBED FLOWRATES` stufenweise (oder in einem {ref}`liquid boundary file <tm2d-liq-file>`) erhöhen oder
