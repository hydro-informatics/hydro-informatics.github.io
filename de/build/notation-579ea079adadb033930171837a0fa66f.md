...
* - $c_{cfl}$
  - $-$
  - Courant-Friedrichs-Lewy ({term}`CFL-Zahl <CFL>`) Bedingung
...
* - $Fr$
  - $-$
  - {term}`Froude-Zahl <Froude number>`
...
* - $f_{eh}$
  - $-$
  - Faktor in der Engelund und Hansen Geschiebefracht Gleichung {eq}`eq-f-eh`
...
* - $Q_{b}$
  - kg s$^{-1}$
  - Geschiebetransport (Kapazität)

* - $Q_{b * cr}$
  - kg s$^{-1}$
  - dimensionsloser Geschiebetransport (Kapazität)
...
* - $q_{b,sc}$
  - kg s$^{-1}$m$^{-1}$
  - {ref}`slope-corrected <gaia-dir>` Geschiebetransport (Kapazität)
...
* - $\theta'$
  - $-$
  - Haut-Reibung-korrigiert {term}`Dimensionslose Schubspannung <Dimensionless bed shear stress>` ($\theta' = \mu\theta$), d.h. die {term}`Shields parameter`-Schere, die in den Suspensionsformeln verwendet wird; entspricht $\tau_{x}$ mit Hautreibungskorrektur (vgl. {ref}`gaia-sl-formulae`)
...
* - $\Phi$
  - $-$
  - dimensionslos {term}`Sedimenttransport <Sediment transport>`

* - $\Phi_b$
  - $-$
  - dimensionless {term}`Geschiebe <Bedload>` transport
...
* - $\tau_x$
  - $-$
  - {term}`Dimensionslose Schubspannung <Dimensionless bed shear stress>`

* - $\tau_{x,cr}$
  - $-$
  - Kritisch {term}`Dimensionslose Schubspannung <Dimensionless bed shear stress>` oder {term}`Shields parameter`
