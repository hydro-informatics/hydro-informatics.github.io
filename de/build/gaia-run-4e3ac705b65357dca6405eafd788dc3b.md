---
description: Leitfaden für die Durchführung von morphodynamischen TELEMAC-GAIA-Simulationen und die Analyse von Sedimenttransportergebnissen einschließlich Geschiebefracht, Schwebstofffracht und Bettentwicklung.
---
...
* Lösung: Verwenden Sie das Schema `14` oder `15` für suspendierte Sedimente, reduzieren Sie den Zeitschritt, überprüfen Sie die Randbedingungen für Schwebstofffracht
```
...
**rGaia-steady2d.slf** enthält die in der Gaia-Lenkdatei definierten morphodynamischen Variablen (z. B. Bodenhöhe, Transportraten der Geschiebefracht, Sedimentkonzentrationen). Die Variablen entsprechen dem Schlüsselwort **VARIABLES FOR GRAPHIC PRINTOUTS** in `gaia-morphodynamics.cas`.
...
| `QSBL` | Transportrate der Geschiebefracht (pro Breiteneinheit) | kg/(m·s) |
...
Also water depth, flow velocity (vectors), and {term}`Topografischer Wandel <Topographic change>` should be analyzed (in QGIS or BlueKenue) since Gaia modifies riverbed elevations. For instance, if the model predicts {term}`Topografischer Wandel <Topographic change>` in the form of 10-m deep erosion (scour) at baseflow, the keyword definitions for the {ref}`riverbed <gaia-active-lyr>` should be revised. Likewise, hydro-morphodynamically relevant parameters such as {ref}`friction <c-friction>`, or {ref}`direction and magnitude (bedload) <gaia-dir>` correctors should be verified.
...
- [ ] Geschieberichtung fluchtet mit Hauptströmungsrichtung (mit erwarteten Kurvenabweichungen)
...
When a model is finally and approximately physically meaningful, the model can be {ref}`calibrated <bl-calibration>` with observation data. The next section provides a list of keywords that may be used for calibrating {term}`Geschiebe <Bedload>` and/or {term}`Schwebstoff <Suspended load>` simulations with Gaia.
...
* **Geschiebe Messungen**: Sampler- oder Fallendaten (selten, aber wertvoll)
...
* Der **MPM COEFFICIENT** kann angepasst werden (Standard: `8`):
  ...
  * Reduzieren Sie, um die Gesamttransportraten für Geschiebefracht zu verringern.
...
**What next?**
: The calibrated model will also require validation. The validation requires another set of riverbed elevation measurements from before and after another flood (i.e., an additional event-specific {term}`Topografischer Wandel <Topographic change>` map). Alas, {term}`Topografischer Wandel <Topographic change>` maps are expensive and it is rare to have at least three {term}`Digitales Oberflächenmodell <DEM>`s from different points in time for a river section, which would enable the creation of two {term}`Topografischer Wandel <Topographic change>` maps. For this reason, the calibration dataset is often split in practice. For instance, 2/3 of a {term}`Topografischer Wandel <Topographic change>` map may be used for model calibration and 1/3 for model validation. However, such splitting makes that the two datasets are not statistically independent and the validation quality figures will be biased.
