...
* Rechtsklicken Sie auf `/dev/sda3` (die Partition hinter der Root-Partition) und klicken Sie auf *Swapoff*.
* Klicken Sie erneut mit der rechten Maustaste auf `/dev/sda3` und klicken Sie auf *Größe/Move*.
+ In dem * Freiraum vor (MiB):* Feld, geben Sie eine angemessene Größe, um den Festplattenplatz für die Root-Partition freizugeben (z.B. `2000`).
+ Stellen Sie sicher, dass die *Neue Größe* und *Freier Speicherplatz nach* Boxen kohärent mit dem verfügbaren Speicherplatz sind, insbesondere wenn Sie gerade die Größe der virtuellen Festplatte erhöhen.
+ Klicken Sie auf * Größe/Bewegung*.
...
+ Stellen Sie sicher, dass es keinen *Freiraum folgt* und dass das Feld mit dem verfügbaren Raum nach der Wurzelpartition kohärent ist.
...
1. Laden und speichern Sie den WineHQ Key in einem neuen Standort: Führen Sie den folgenden Befehl aus, um den Schlüssel herunterzuladen, es zu speichern und in den empfohlenen Ort zu speichern (z.B. `/usr/share/keyrings/winehq-archive.key`):
...
2. Update der Repository-Konfiguration
...
* In QGIS bearbeiten Sie die Bearbeitungsoptionen (Anbieter > Werkzeuge für LiDAR-Daten):
* Ordner LasTools: Geben Sie hier den Pfad in den Ordner ein, in dem die extrahierten ZIP-Dateien leben (`/dir/to/LAStools/`)
* Weinordner: Geben Sie den Pfad an die {ref}`wine` binär (typischerweise `/usr/bin/`) ein. Alternativ finden Sie, wo Wein mit dem `whereis wine`-Befehl lebt
...
* Paketinformationen aktualisieren und Ihr System:
...
2. Starten Sie ein neues Pojekt mit einem Klick auf **Data Projekt**.
...
4. Klicken Sie *****, wählen Sie ein Ausgabeverzeichnis und ändern Sie optional den Ausgabebildnamen.
...
2. ** Einen Befestigungspunkt für die ISO:**
...
1. ** Überprüfen Sie die Startskripte oder Dokumentation:**
