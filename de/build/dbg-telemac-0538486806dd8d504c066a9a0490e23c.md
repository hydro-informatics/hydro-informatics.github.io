...
- GRETEL nimmt die partiellen (Subdomain) Ergebnisdateien aus jedem Kern und führt sie zurück in eine einzelne SELAFIN/MED-Datei für die gesamte Domäne.
- Dies ist in der Regel der **go‐to-Ansatz**, wenn Ihre Simulation teilweise durchstürzt, aber Sie wollen immer noch visualisieren oder analysieren, was Teilergebnisse gespeichert wurden.

2. **PARTEL (im Fusionsmodus)**
- PARTEL wird in der Regel verwendet, um Maschen für Parallelläufe zu trennen, aber es hat auch eine Zusammenführungsfähigkeit. Einige Versionen oder Workflows verwenden PARTEL für die Partitionierung und Verschmelzung.
- Für die meisten modernen Telemac-Konfigurationen neigt **GRETEL** dazu, das schlankere oder Standard-Merging-Tool zu sein.

### Wie man GRETEL verwendet

1. Wenn Sie Ihre Simulation parallel lief und es abgestürzt (oder abgeschlossen, aber Sie müssen nur Subdomains manuell kombinieren), finden Sie die Teil-Ergebnis-Dateien benannt etwas wie:
   ```
...
* * *Connectivité des éléments?* was bedeutet in <br>Englisch: **Element-Konnektivität?**
+ Option `1`
+ Option `2`
...
+ In der 4. Zeile nach dem Laufen `./salome` kann `Kernel/Session` anfordern
...
```
+ Einfügen in (bestätigen Sie den Austausch `libQtCore.so.5`)
...
+ Setzen Sie *Style* an `gtk2` und *Standarddialoge* an `GTK2`
+ Klick auf *Apply* und *OK*
