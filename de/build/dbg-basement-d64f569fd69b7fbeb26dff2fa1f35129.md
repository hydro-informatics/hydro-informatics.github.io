...
1. Verwenden Sie das Find-and-Replace-Tool (`CTRL` + `H` Tasten in Notepad++), um Dateipfade vor `results_aux.h5` zu entfernen.
* Suchen Sie den `results_aux.h5`-String und identifizieren Sie den vor ihm geschriebenen Pfad (z.B. `C:\temp\`).
* Diesen Benutzerpfad finden und ersetzen, z.B.: `Find` = `C:\temp\results_aux.h5` und `Replace with` = `results_aux.h5`.
