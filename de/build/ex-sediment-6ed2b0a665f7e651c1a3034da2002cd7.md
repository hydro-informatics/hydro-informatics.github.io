...
wenn
*$n_m$ ist der [*Manning*-Koeffizient](http://www.fsl.orst.edu/geowater/FX3/help/8_Hydraulic_Reference/Mannings_n_Tables.htm) in *fictional*-Einheiten (s/m$^{1/3}$)
* $S_{e}$ ist die hypothetische Energieflanke (m/m) und entspricht der Kanalneigung für stetige, einheitliche Strömungsbedingungen (nicht vorhanden in natürlichen Flüssen).
...
- der benetzte Umfang eines Trapezoids ist $P = b + 2h\cdot(m^2 + 1)^{1/2}$;
...
:name: qb
$$
Q {b} = \Phi b \cdot ((s-1) \cdot g \cdot D {char}^{3}^{1/2} \cdot \rho {{}
$$
```
...
:name: qbx
$$
Q {b} = b {eff} \cdot q {b} = b {eff} \cdot \Phi b \cdot [(s-1) \cdot g \cdot D {char}{3}^{1/2} \cdot \rho {{{{}
$$
```
